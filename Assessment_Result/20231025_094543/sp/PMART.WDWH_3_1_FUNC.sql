REPLACE PROCEDURE PMART.WDWH_3_1_FUNC
(
...
SP:BEGIN
  ...
  CALL PMART.REMD_SCAL_DEPT_FUNC(I_DAY_ID);
  ...
  END IF ;       
  CALL PMART.P_DROP_TABLE ('#VT_WDWH_3_1_FUNC');
  IF (I_ORG_TYPE = '-1') THEN
      CREATE MULTISET VOLATILE TABLE #VT_WDWH_3_1_FUNC AS
      (SELECT * FROM 
         (SELECT A.*, B.*                                                        
            FROM (                                                                 
