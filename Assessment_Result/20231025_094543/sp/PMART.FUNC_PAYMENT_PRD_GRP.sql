...
SP:BEGIN
   DECLARE SQLSTR  VARCHAR(5000)  ; 
	...
	INSERT INTO PMART.T1(F1,F2) SELECT 2,SQLSTR;
	EXECUTE IMMEDIATE SQLSTR; 	
	...
	SET SQLSTR = SQLSTR  +'                        ,DAT.GRP_ID'
				                          ...
				                          +'                        ,(CAST ( (1.0000 * DAT.PAY_TYPE_AMT/DAT_TX.TX_AMT) AS DECIMAL(20,4) ) * 100) PAY_TYPE_RATIO '
				                          +'                        ,(CAST ( (1.0000 * DAT.ALL_PAY_TYPE_AMT/DAT_TX.TX_AMT) AS DECIMAL(20,4) ) * 100) ALL_PAY_TYPE_RATIO '
					                      +'                        ,(CAST ( (1.0000 * DAT.PAY_TYPE_AMT/DAT.ALL_PAY_TYPE_AMT) AS DECIMAL(20,4) ) * 100) AMT_IN_ALL_RATIO '
				                          +'       FROM '
				                          +'       (	'
	 ...
	 IF (P_GRP_ID ='-1') THEN 					  
                          SET SQLSTR = SQLSTR + '   AND LENGTH(GRP_ID)=2  ';
	 ELSEIF ( LENGTH(P_GRP_ID) = 2 ) THEN 					  
	                      SET  SQLSTR = SQLSTR +'    AND LENGTH(GRP_ID)=3   '
						                                        +'    AND SUBSTRING(GRP_ID,1,2) = '''+P_GRP_ID+''''; 
	 ELSE 
	 ...
	 SET SQLSTR = SQLSTR+'                             GROUP BY  GRP_ID   '
                                         ...
                                         +'     				                              FROM SYSDBA.#VT_IGA_2_4_PRDGRP_TX '
                                         +'     			                             ) B  '
										 +'                           )DAT_TX ON 1=1 ';
	IF (P_GRP_ID = '-1') THEN 					  
	...
	DELETE FROM PMART.T1 WHERE F1 = 3;
	INSERT INTO PMART.T1(F1,F2) SELECT 3,SQLSTR;
    EXECUTE IMMEDIATE SQLSTR;   
