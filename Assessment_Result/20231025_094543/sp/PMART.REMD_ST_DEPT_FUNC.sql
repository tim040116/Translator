REPLACE PROCEDURE PMART.REMD_ST_DEPT_FUNC(P_DAY_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_ST_DEPT_FUNC'); 
  CALL PMART.REMD_S1_DEPT_FUNC(P_DAY_ID);
  CALL PMART.REMD_S2_DEPT_FUNC(P_DAY_ID);
  CALL PMART.REMD_S3_DEPT_FUNC(P_DAY_ID);
  CALL PMART.REMD_S4_DEPT_FUNC(P_DAY_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_ST_DEPT_FUNC AS('+            
          ' SELECT '+
          ' -1 AS TOT_ID, '+
          ' A.ORG_ID AS ORG_ID, '+
          ...
          ' FROM '+
          ' #VT_REMD_S1_DEPT_FUNC  A '+
          ' LEFT JOIN '+
          ' #VT_REMD_S2_DEPT_FUNC  B '+
          '    ON (A.ORG_ID=B.ORG_ID) '+
          ' LEFT JOIN '+
          ' #VT_REMD_S3_DEPT_FUNC  C '+
          '    ON (A.ORG_ID=C.ORG_ID) '+
          ' LEFT JOIN '+
          ' #VT_REMD_S4_DEPT_FUNC  D '+
