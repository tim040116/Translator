REPLACE PROCEDURE PMART.OLD0401_WDWH_4_1_FUNC(P_L_DAY_ID VARCHAR(400),P_DAY_TYPE VARCHAR(1), P_LEVEL VARCHAR(1), P_PRD_ID VARCHAR(400), P_DEPT_ID INTEGER, P_KND VARCHAR(2))
...
SP:BEGIN
   ...
   ELSE
	    ...
	    SET SQLSTR = '  CREATE MULTISET VOLATILE TABLE #VT_WDWH_4_1_FUNC_3 AS 	(		'
								...
								+ '	   WHERE CAST(TX_DTTM AS DATE)= CAST(CAST(' +''''+ P_L_DAY_ID+''''+ ' AS INTEGER)-19000000 AS DATE) AND OSTORE_ID IN (SELECT OSTORE_ID FROM  PMART.ORG_DIM) 	GROUP BY OSTORE_ID	) S  '
								+ '	 ) WITH DATA NO PRIMARY INDEX ON COMMIT PRESERVE ROWS; ';
		LOCKING TABLE PDATA.TRANS_PRODUCT_DETAIL FOR ACCESS;
		 EXECUTE IMMEDIATE SQLSTR;
		 SET SQLSTR = '';
   END IF;
   INSERT INTO PMART.T1(F1,F2) SELECT 9,'02 ' + CAST(CURRENT_TIMESTAMP AS CHAR(30));
   ...
   CASE P_LEVEL
         WHEN '4' THEN 
		   IF V_D_FLAG = 'F'  THEN
			    CREATE MULTISET VOLATILE TABLE #VT_WDWH_4_1_FUNC_1 AS 
				(
				 	  SELECT  '2' AS FLAG, P.GRP_ID AS PRD_ID_1, ISNULL(SUM(T.SALES_AMT)) AS NOW_SALES_AMT, ISNULL(SUM(T.SALES_CNT)) AS NOW_SALES_CNT
						...
						UNION
				 		SELECT  '1' AS FLAG, B.RESPON_ID AS PRD_ID_1, ISNULL(SUM(T.SALES_AMT)) AS NOW_SALES_AMT, ISNULL(SUM(T.SALES_CNT)) AS NOW_SALES_CNT
						FROM  (SELECT DISTINCT RESPON_ID,GRP_ID FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1 ) B
						LEFT JOIN  PMART.BASIC_MFACT_BUDGET T ON T.PRD_ID  = B.GRP_ID AND T.TIME_ID = :V_C_DATE  
						INNER JOIN PMART.PRD_GRP AS P ON P.GRP_ID=T.PRD_ID
		   ...
		   ELSE
				 ...
				 CREATE MULTISET VOLATILE TABLE #VT_WDWH_4_1_FUNC_1 AS	
				 (
				 		SELECT  '2' AS FLAG, P.GRP_ID AS PRD_ID_1,  ISNULL(CAST(SUM((Z.AM_ITEM-Z.AM_DIS_SUB-Z.AM_SUB_SUB)*DWH_FG)AS DECIMAL(16,6))/S.STORE_NUM) AS NOW_SALES_AMT, ISNULL(CAST(SUM(Z.QT_ITEM*DWH_FG)AS DECIMAL(16,6))/S.STORE_NUM) AS NOW_SALES_CNT
						FROM  (SELECT DISTINCT RESPON_ID,GRP_ID FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1 ) B
						...
						UNION
				 		SELECT  '1' AS FLAG, B.RESPON_ID AS PRD_ID_1,  ISNULL(CAST(SUM((Z.AM_ITEM-Z.AM_DIS_SUB-Z.AM_SUB_SUB)*DWH_FG)AS DECIMAL(16,6))/S.STORE_NUM) AS NOW_SALES_AMT, ISNULL(CAST(SUM(Z.QT_ITEM*DWH_FG)AS DECIMAL(16,6))/S.STORE_NUM) AS NOW_SALES_CNT
						FROM  (SELECT DISTINCT RESPON_ID,GRP_ID FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1 ) B
                        LEFT JOIN PMART.PRD_DIM P ON P.GRP_ID = B.GRP_ID 
						LEFT JOIN  PDATA.TRANS_PRODUCT_DETAIL  Z ON P.PRD_ID = Z.CD_FMCODE AND CAST(Z.TX_DTTM AS DATE) = CURRENT_DATE
						LEFT JOIN #VT_WDWH_4_1_FUNC_3 AS S ON 1=1	 
		   ...
		   END IF;	
			 CREATE MULTISET VOLATILE TABLE #VT_WDWH_4_1_FUNC_2 AS (
			 		SELECT  B.RESPON_ID AS PRD_ID_2,
					                CASE WHEN MAX(UPLOAD_STNUM) > 0 THEN  ISNULL(SUM(SALES_AMT-DIS_AMT-SUB_AMT)/MAX(CAST(UPLOAD_STNUM AS DECIMAL(16,6)))) ELSE 0 END AS REAL_SALES_AMT_M_PSD,
         ...
         WHEN '3' THEN 
		   IF V_D_FLAG = 'F'  THEN
			    CREATE MULTISET VOLATILE TABLE #VT_WDWH_4_1_FUNC_1 AS 
				(
				 	  SELECT  '2' AS FLAG, P.KND_ID AS PRD_ID_1, ISNULL(SUM(T.SALES_AMT)) AS NOW_SALES_AMT, ISNULL(SUM(T.SALES_CNT)) AS NOW_SALES_CNT
						FROM  (SELECT DISTINCT RESPON_ID,GRP_ID FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1 ) B
						LEFT JOIN  PMART.BASIC_MFACT_BUDGET T ON T.PRD_ID  = B.GRP_ID AND T.TIME_ID = :V_C_DATE 
						INNER JOIN PMART.PRD_GRP AS P ON P.GRP_ID=T.PRD_ID
						WHERE B.RESPON_ID = P_DEPT_ID 
		   ...
		   ELSE
				 ...
				 CREATE MULTISET VOLATILE TABLE #VT_WDWH_4_1_FUNC_1 AS
				 (
				 		SELECT  '2' AS FLAG, P.KND_ID AS PRD_ID_1,  ISNULL(CAST(SUM((Z.AM_ITEM-Z.AM_DIS_SUB-Z.AM_SUB_SUB)*DWH_FG)AS DECIMAL(16,6))/S.STORE_NUM) AS NOW_SALES_AMT, ISNULL(CAST(SUM(Z.QT_ITEM*DWH_FG)AS DECIMAL(16,6))/S.STORE_NUM) AS NOW_SALES_CNT
						FROM  (SELECT DISTINCT RESPON_ID,GRP_ID FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1 ) B
                        LEFT JOIN PMART.PRD_DIM P ON P.GRP_ID = B.GRP_ID 
						LEFT JOIN  PDATA.TRANS_PRODUCT_DETAIL  Z ON P.PRD_ID = Z.CD_FMCODE AND CAST(Z.TX_DTTM AS DATE) = CURRENT_DATE
						LEFT JOIN #VT_WDWH_4_1_FUNC_3 AS S ON 1=1	
         ...
         WHEN '2' THEN 
		   ...
		   END IF;		   
			 CREATE MULTISET VOLATILE TABLE #VT_WDWH_4_1_FUNC_2 AS (
			 		SELECT  B.BRANCH_ID AS PRD_ID_2,
					        CASE WHEN MAX(UPLOAD_STNUM) > 0 THEN ISNULL(SUM(SALES_AMT-DIS_AMT-SUB_AMT)/MAX(CAST(UPLOAD_STNUM AS DECIMAL(16,6)))) ELSE 0 END AS REAL_SALES_AMT_M_PSD,
							CASE WHEN MAX(UPLOAD_STNUM) > 0 THEN ISNULL(SUM(SALES_CNT)/MAX(CAST(UPLOAD_STNUM AS DECIMAL(16,6)))) ELSE 0 END AS REAL_SALES_CNT_M_PSD
					FROM PMART.BASIC_MFACT_BUDGET T
					INNER JOIN PMART.PRD_GRP AS P ON P.GRP_ID=T.PRD_ID
	  ...
	  WHEN '0' THEN 
		   ...
		   ELSE
				 ...
				 (
						...
						LEFT JOIN  PDATA.TRANS_PRODUCT_DETAIL  Z ON CAST(P.FM_CODE AS INTEGER) = Z.CD_FMCODE AND CAST(Z.TX_DTTM AS DATE) = CURRENT_DATE
						LEFT JOIN #VT_WDWH_4_1_FUNC_3 AS S ON 1=1	
   ...
   INSERT INTO PMART.T1(F1,F2) SELECT 9,'03 ' + CAST(CURRENT_TIMESTAMP AS CHAR(30));
	EXECUTE IMMEDIATE SQLSTR;
   INSERT INTO PMART.T1(F1,F2) SELECT 9,'04 ' + CAST(CURRENT_TIMESTAMP AS CHAR(30));
END SP;