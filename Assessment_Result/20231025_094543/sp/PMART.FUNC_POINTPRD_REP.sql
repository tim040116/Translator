REPLACE PROCEDURE PMART.FUNC_POINTPRD_REP
(
...
SP:BEGIN
    ...
    CALL PMART.P_DROP_TABLE ('#VT_FUNC_POINTPRD_REP');
    SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_FUNC_POINTPRD_REP ( '
    ...
    SET SQLSTR = 'INSERT INTO #VT_FUNC_POINTPRD_REP(PARENT_PRD_ID,PRD_ID,FM_NAME,ALLOW_DRILL_DOWN,PRD_LEVEL,DRILL_PRD_LEVEL,DRILL_PRD_ID) '
	                     +'SELECT * FROM( ';
    IF P_SHOWTYPE = 2 THEN 	
		   SET SQLSTR = SQLSTR +'  SELECT DAT.*,CASE WHEN PRD_LEVEL=1 THEN -1 ELSE (PRD_LEVEL-1) END AS DRILL_PRD_LEVEL,DAT.PARENT_PRD_ID AS DRILL_PRD_ID '
		                                    +'     FROM PMART.VW_POINTPRD_REP DAT '
