REPLACE PROCEDURE PMART.REMD_OT_STYPE_FUNC(P_DAY_ID NUMBER,P_STYPE_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(10000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_OT_STYPE_FUNC'); 
  CALL PMART.REMD_O1_STYPE_FUNC(P_DAY_ID,P_STYPE_ID);
  CALL PMART.REMD_O2_STYPE_FUNC(P_DAY_ID,P_STYPE_ID);
  CALL PMART.REMD_O3_STYPE_FUNC(P_DAY_ID,P_STYPE_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_OT_STYPE_FUNC AS('+            
               'SELECT '+
               '-1 AS TOT_ID, '+
               ...
               'FROM '+
               '#VT_REMD_O1_STYPE_FUNC A '+
               'LEFT JOIN '+
               '#VT_REMD_O2_STYPE_FUNC B '+
               '   ON (A.OSTORE_ID=B.OSTORE_ID) '+
               'LEFT JOIN '+
               '#VT_REMD_O3_STYPE_FUNC C '+
