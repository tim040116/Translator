REPLACE PROCEDURE PMART.B2BIGA_1_6_FUNC
(
...
SP:BEGIN
   ...
   DECLARE V_TIME_STR VARCHAR(400); 
	CALL PMART.P_DROP_TABLE ('#VT_B2BIGA_1_6_FUNC'); 
	IF P_TIME_TYPE='M' THEN
		SET V_TABLE_NAME = ' PMART.S_MONTH ';
	...
	END IF;
   SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_B2BIGA_1_6_FUNC  AS( '
        + ' SELECT B.CMNO AS CMNO, '
		+ ' C.FM_NAME AS CMNM, '
		+ ' B.MRKTKD AS LOCAL1, '
		+ ' B.COUNT1 AS COUNT1 '
		+ ' FROM ( '
		+ ' SELECT A.CMNO, A.MRKTKD, SUM(COUNT1) AS COUNT1 '
		+ ' FROM '+ V_TABLE_NAME +' A '
		+ ' WHERE '+ V_TIME_STR +' ' 
		+ ' AND '+ V_PRD_ID_STR +' '
		+ ' GROUP BY A.CMNO, A.MRKTKD) B, '
		+ ' PDATA.PBMCMDT C '
