CREATE PROCEDURE PMART.NEW_TPO_MFACT_1
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_NEW_TPO_MFACT_1'); 
   CASE
      WHEN FP_TIME_TYPE='D' AND FP_PRD_TYPE='P' AND FP_PRD_LEVEL=2  AND FP_ORG_LEVEL=4  THEN  
            SET V_TABLE_NAME = 'PMART.BASIC_MFACT_DETAIL';             
      WHEN FP_TIME_TYPE='D' AND FP_PRD_TYPE='R' AND FP_PRD_LEVEL=1  AND FP_ORG_LEVEL=4  THEN  
            SET V_TABLE_NAME = 'PMART.BASIC_MFACT_DETAIL';
   ...
   END IF;
   SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_NEW_TPO_MFACT_1  AS('+
                  'SELECT '+
