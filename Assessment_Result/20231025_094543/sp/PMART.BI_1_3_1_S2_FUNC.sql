REPLACE PROCEDURE PMART.BI_1_3_1_S2_FUNC(P_TIME_ID NUMBER,P_LEVEL NUMBER,P_ORG_ID NUMBER,P_PRD_ID VARCHAR(7),P_PRD_VALUE NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
  DECLARE P_TIME_ID_L NUMBER;
   DECLARE SQLSTR VARCHAR(1000) DEFAULT '';
   DECLARE ORG_STR VARCHAR(400) DEFAULT '';
   DECLARE TAB_STR VARCHAR(100)  DEFAULT '';
   DECLARE STORE_CS CURSOR FOR STORE_SQL;
   IF (P_LEVEL=-1) THEN  
      SET ORG_STR = ' SELECT DISTINCT STORE_ID FROM PMART.LAST_ORG_DIM WHERE TOT_ID=-1 ';
   END IF;
   IF (P_LEVEL=0) THEN  
      SET ORG_STR = ' SELECT DISTINCT STORE_ID FROM PMART.LAST_ORG_DIM WHERE PDEPT_ID= '+P_ORG_ID;
   END IF;
   IF (P_LEVEL=1) THEN  
   ...
   END IF;
   SET SQLSTR = ' SELECT DISTINCT B.L_WEEK_ID ' +
                ' FROM PMART.YMWD_TIME A, PMART.YMWD_TIME B ' +                
                ' WHERE A.L_WEEK_ID = '+P_TIME_ID +
		        ' AND B.L_WEEK_SERIAL = A.L_WEEK_SERIAL-1 ' +
                ' ' ;	              		
  ...
  DO    
     L1_1:
        BEGIN 		
         FETCH STORE_CS INTO P_TIME_ID_L ;         
         IF SQLSTATE <> '00000' THEN 
            LEAVE L1; 
         END IF;  	   
     END L1_1;     
  END WHILE L1;
      CALL PMART.P_DROP_TABLE ('#VT_BI_1_3_1_S2_FUNC');  
      SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_BI_1_3_1_S2_FUNC  AS('+
