REPLACE PROCEDURE PMART.OLD0401_FUNC_FACT_FESTIVAL
(
...
SP:BEGIN
   ...
   END IF;   
   	   IF P_PRDTYPE = '-1' THEN			   
	        SET PRDJOIN = ' JOIN PDATA.FESTL_PRD_SET T3 ON T3.ACT_ID = ' + P_ACTID + ' AND T1.PRD_ID = CAST(T3.FM_CODE AS INTEGER) ';
			SET PRDANDWHERE = '';			
   ...
   IF POSITION('PRD_ID' IN P_GROUP) <> 0 THEN   		
			IF P_PRDTYPE = '-1' THEN
				SET P_GROUP_COL = ' TIME_ID, T3.CLASS_ID AS PRD_ID ';
				SET P_GROUP_BY = ' TIME_ID, T3.CLASS_ID ';
		   ELSEIF P_PRDTYPE = '1' THEN   
				SET P_GROUP_COL = ' TIME_ID, T3.DIVISION_ID AS PRD_ID ';
