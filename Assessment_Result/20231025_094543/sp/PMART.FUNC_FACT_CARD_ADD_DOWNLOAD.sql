REPLACE PROCEDURE PMART.FUNC_FACT_CARD_ADD_DOWNLOAD
(
...
SP:BEGIN
   ...
   DECLARE FGADDTYPEWHERE  VARCHAR(200);  
   IF P_ORGTYPE = '0' THEN
   ...
   IF P_FGADDTYPE = '' THEN
   	   SET FGADDTYPEWHERE = '';
   ELSE
       SET FGADDTYPEWHERE = ' AND T1.FG_ADD_TYPE IN ('+P_FGADDTYPE+') ';
   END IF;	   
CALL PMART.P_DROP_TABLE ('#VT_FACT_CARD_ADD_DOWNLOAD');
SET SQLSTR = ' CREATE MULTISET VOLATILE TABLE #VT_FACT_CARD_ADD_DOWNLOAD AS('                          										   
						   + ' SELECT T1.ORG_ID, T1.TIME_ID, T1.CARD_TYPE, T1.FG_ADD_TYPE, T1.FG_ADD_RANGE, '
						   + ' 	            T2.DEPT_NO, T2.BRANCH_NO, T2.STORE_NO, T2.STORE_NAME, ' + TIMENAME
						   + ' 				T4.CODE_NAME FG_ADD_TYPE_NM,  '
						   + ' 				T5.CODE_NAME CARD_TYPE_NM, '
						   + ' 				T6.CODE_NAME FG_ADD_RANGE_NM, '
						   + ' 				T1.ADD_AMT, T1.ADD_CNT, T2.PDEPT_NO '
						   + ' 	   FROM PMART.FACT_CARD_ADD T1 '
						   + ' 	    JOIN ( '
						   ...
						   +        TIMEWHERE						   
						   + '  	 JOIN PMART.VW_PBMCODE T4 ON T1.FG_ADD_TYPE = T4.CODE_ID AND T4.CODE_TYPE = ''FG_ADD_TYPE'' '
						   + ' 	 JOIN PMART.VW_PBMCODE T5 ON T1.CARD_TYPE = T5.CODE_ID AND T5.CODE_TYPE = ''CARD_TYPE'' '
						   + ' 	 JOIN PMART.VW_PBMCODE T6 ON T1.FG_ADD_RANGE = T6.CODE_ID AND T6.CODE_TYPE = ''FG_ADD_RANGE'' '
						   + '     WHERE T1.TIME_ID IN ('+P_TIMEID+') '
						   +          CARDTYPEWHERE
						   +          FGADDTYPEWHERE
                           + ' ) WITH DATA PRIMARY INDEX(ORG_ID, TIME_ID, FG_ADD_TYPE, CARD_TYPE, FG_ADD_RANGE) ON COMMIT PRESERVE ROWS;';
