REPLACE PROCEDURE PMART.POSN_MFACT_1_9_1
(
FP_TIME_TYPE CHAR,         
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_POSN_MFACT_1_9_1'); 
   SET V_TIME_SQL = ' ';
   SET V_LATEST_SHOPDIST = 'SELECT DISTINCT O.OSTORE_ID AS ORG_ID,O.OSTORE_ID AS OSTORE_ID,O.BRANCH_ID,O.DEPT_ID,O.PDEPT_ID ,O.TOT_ID FROM PMART.LAST_SHOPDIST AS L, PMART.LATEST_ORG_DIM AS O WHERE L.OSTORE_NO = O.OSTORE_ID ';
  ...
  END IF;
   ...
   IF (FP_TIME_TYPE = 'W' OR FP_TIME_TYPE = 'M' ) THEN 
      SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSN_MFACT_1_9_1  AS('+
                  'SELECT  '+
