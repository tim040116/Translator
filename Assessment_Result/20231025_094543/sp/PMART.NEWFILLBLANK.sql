REPLACE PROCEDURE PMART.NEWFILLBLANK (P_MONTH_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
MERGE INTO PMART.REMD_FACT T USING
(
SELECT
...
,0           AS STATUS
,0           AS AC
,B.SLDAY     AS BUDGET_ST_TOT_PLAN_STNUM
,C.SLDAY     AS BUDGET_DEPT_TOT_PLAN_STNUM
,E.AMT       AS BUDGET_PDEPT_AVG_AMT
,E.TAMT      AS BUDGET_PDEPT_TOT_AMT
,E.SLDAY     AS BUDGET_PDEPT_TOT_PLAN_STNUM
FROM
(
  SELECT
...
LEFT OUTER JOIN
(
  SELECT OSTORE_ID,YYYYMM,AVG(AMT) AS AMT,SUM(TAMT) AS TAMT,SUM(SLDAY) AS SLDAY
    FROM PDATA.ST_REMD1400M 
...
(
   ...
   ,T1.TAMT AS TAMT
   ,T1.SLDAY AS SLDAY   
     FROM PDATA.ST_REMD1400Y T1,PMART.LAST_ORG_DIM T2
