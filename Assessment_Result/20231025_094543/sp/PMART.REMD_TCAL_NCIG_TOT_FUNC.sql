REPLACE PROCEDURE PMART.REMD_TCAL_NCIG_TOT_FUNC(P_DAY_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_TCAL_NCIG_TOT_FUNC'); 
  CALL PMART.REMD_TT_NCIG_TOT_FUNC(P_DAY_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_TCAL_NCIG_TOT_FUNC AS('+            
                'SELECT '+
                'TOT_ID, '+
                'ORG_ID, '+
		...
		'SELECT ((SUM(ACCU_AMT) / SUM((DECODE(ACCU_PLAN_STNUM,0,NULL,ACCU_PLAN_STNUM) * DECODE(BUDGET_DEPT_AVG_AMT,0,NULL,BUDGET_DEPT_AVG_AMT)))) * 0.9561 * 100) AS AMT '+		
	        ...
	        ' ) AS R6,  '+
                ...
                'FROM #VT_REMD_TT_NCIG_TOT_FUNC T1 '+
