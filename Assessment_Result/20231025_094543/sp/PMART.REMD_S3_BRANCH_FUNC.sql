REPLACE PROCEDURE PMART.REMD_S3_BRANCH_FUNC(P_DAY_ID NUMBER,P_DEPT_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_S3_BRANCH_FUNC'); 
  LOCKING PMART.REMD_FACT_SUM FOR ACCESS;
  LOCKING PMART.LAST_ORG_DIM FOR ACCESS;  
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_S3_BRANCH_FUNC AS('+            
               'SELECT '+
               ...
               '(SELECT DISTINCT BRANCH_ID FROM PMART.LAST_ORG_DIM '+
                'WHERE DEPT_ID='+P_DEPT_ID+') B '+
