REPLACE PROCEDURE PMART.REMD_T1_OSTYPE_FUNC(P_DAY_ID NUMBER,P_ORG_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_T1_OSTYPE_FUNC'); 
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_T1_OSTYPE_FUNC AS('+ 
               'SELECT '+
               ...
               '(SELECT DISTINCT OSTYPE_ID FROM PMART.STORE_TYPE_DIM) B '+
               'WHERE A.STYPE_ID=B.OSTYPE_ID AND '+
