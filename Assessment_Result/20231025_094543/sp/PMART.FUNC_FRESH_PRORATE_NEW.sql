REPLACE PROCEDURE PMART.FUNC_FRESH_PRORATE_NEW
(
...
SP:BEGIN
	...
	END IF; 
        DELETE FROM PTEMP.T2 WHERE F1 >= 1 ;
	INSERT INTO PTEMP.T2(F1,F2) SELECT 1,ORGWHERE;	
	INSERT INTO PTEMP.T2(F1,F2) SELECT 2,DAYWHERE;		
	...
	SET SQLSTR = ' CREATE MULTISET VOLATILE TABLE #VT_DAYCNT AS('
		  + ' SELECT DIM.STORE_ID,DIM.STORE_NM,DIM.DEPT_ID,DIM.DEPT_NM'
		  ...
		  + ' INNER JOIN PMART.LAST_ORG_DIM DIM ON FACT.STORE_ID=DIM.STORE_ID'
         	  + ' INNER JOIN PMART.LAST_STORE_TYPE_DIM LST ON FACT.STORE_ID=LST.STORE_ID'
		  ...
		  + 'GROUP BY DIM.STORE_ID,DIM.STORE_NM,DIM.DEPT_ID,DIM.DEPT_NM, DIM.BRANCH_ID,DIM.BRANCH_NM, TP.STTP_TYPE,TP.STTP_TYPE_NM,E.PRORATE'
                  + ' ) WITH DATA PRIMARY INDEX(STORE_ID) ON COMMIT PRESERVE ROWS;';					  
	INSERT INTO PTEMP.T2(F1,F2) SELECT 31,ORGWHERE1;
	INSERT INTO PTEMP.T2(F1,F2) SELECT 3,SQLSTR;
	...
	INSERT INTO PTEMP.T2(F1,F2) SELECT 4,SQLSTR;
	EXECUTE IMMEDIATE SQLSTR;  
	...
	INSERT INTO PTEMP.T2(F1,F2) SELECT 5,SQLSTR;
	EXECUTE IMMEDIATE SQLSTR;
	...
	INSERT INTO PTEMP.T2(F1,F2) SELECT 6,SQLSTR;
	EXECUTE IMMEDIATE SQLSTR;
	...
	SET SQLSTR = ' CREATE MULTISET VOLATILE TABLE #VT_FUNC_FRESH_PRORATE_3 AS('
		  ...
		  + '  GROUP BY A.DEPT_ID,A.DEPT_NM,A.BRANCH_ID,A.BRANCH_NM,A.STTP_TYPE,A.STTP_TYPE_NM,A.FMCODE,A.FMNAME '
                  + ' ) WITH DATA NO PRIMARY INDEX ON COMMIT PRESERVE ROWS; '; 
	INSERT INTO PTEMP.T2(F1,F2) SELECT 7,SQLSTR;
	EXECUTE IMMEDIATE SQLSTR;
	...
	SET SQLSTR = ' CREATE MULTISET VOLATILE TABLE #VT_FUNC_FRESH_PRORATE_4 AS('
		  ...
		  + '  GROUP BY A.DEPT_ID,A.DEPT_NM,A.BRANCH_ID,A.BRANCH_NM,A.STTP_TYPE,A.STTP_TYPE_NM,A.FMCODE,A.FMNAME '
                  + ' ) WITH DATA NO PRIMARY INDEX ON COMMIT PRESERVE ROWS; '; 
	INSERT INTO PTEMP.T2(F1,F2) SELECT 8,SQLSTR;
	EXECUTE IMMEDIATE SQLSTR;	
	...
	SET SQLSTR = ' CREATE MULTISET VOLATILE TABLE #VT_FUNC_FRESH_PRORATE AS('
		  ...
		  + ',A.DEPT_ID,A.DEPT_NM,A.BRANCH_ID,A.BRANCH_NM,A.STTP_TYPE,A.STTP_TYPE_NM ' 
		  + ORGWHERE3
		  + ',(A.PROFIT*B.PRORATE/100/C.PLAN_STNUM ) AS PSD '
		  + '   FROM #VT_FUNC_FRESH_PRORATE_2  A INNER JOIN #VT_FUNC_FRESH_PRORATE_3 B '
                  ...
                  + ' ) WITH DATA NO PRIMARY INDEX ON COMMIT PRESERVE ROWS; '; 
	INSERT INTO PTEMP.T2(F1,F2) SELECT 9,SQLSTR;
	EXECUTE IMMEDIATE SQLSTR;	
END SP;