REPLACE PROCEDURE PMART.POSM1_2_SUM_FUNC(P_DATE_TYPE VARCHAR(2000),
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_POSM1_2_SUM_FUNC'); 
   ...
   IF (P_DATE_TYPE='D') THEN
      SET V_STRING = 'SELECT M.TIME_ID, M.PRD_ID, '+
            V_SQL+
            'FROM '+
            '(    SELECT 1 AS TIME_ID, M.PRD_ID,'+ V_MR_SUM +' FROM PMART.BASIC_MFACT_MR M , (SELECT DISTINCT L_DAY_ID FROM PMART.YMWD_TIME_W2) B '+
            '     WHERE M.TIME_ID = B.L_DAY_ID AND B.L_DAY_ID IN('+ P_L_DAY_ID +') '+
            '     AND M.MR_ID='+ P_MR_ID +' AND M.PRD_ID IN('+ P_PRD_ID +') GROUP BY M.PRD_ID'+
            ') M, '+
            '('+
            '     SELECT 1 AS TIME_ID, SUM(BIT_EXTRACT(BIT_AND(A.STNUM_STORE_NUM,Y.MASK))) AS U_NUM  '+
            '     FROM PMART.BASIC_MAST_FACT A,PMART.LAST_ORG_DIM_MASK Y,(SELECT DISTINCT L_DAY_ID FROM PMART.YMWD_TIME_W2) B '+
            '     WHERE A.TIME_ID=B.L_DAY_ID AND B.L_DAY_ID IN('+ P_L_DAY_ID +') AND Y.ORG_ID=-1 '+
            ') U '+
   ...
   IF (P_DATE_TYPE='W') THEN
      SET V_STRING = 'SELECT M.TIME_ID, M.PRD_ID, '+
            V_SQL+
            'FROM '+
            '(    SELECT 1 AS TIME_ID, M.PRD_ID,'+ V_MR_SUM +' FROM PMART.BASIC_MFACT_MR M , (SELECT DISTINCT L_DAY_ID,L_WEEK_ID FROM PMART.YMWD_TIME_W2) B '+
            ...
            '     WHERE A.TIME_ID=B.L_DAY_ID AND B.L_WEEK_ID IN('+ P_L_DAY_ID +') AND Y.ORG_ID=-1 '+
            ') U '+
            'WHERE M.TIME_ID=U.TIME_ID ';
   END IF;
   IF (P_DATE_TYPE='M') THEN
      SET V_STRING = 'SELECT M.TIME_ID, M.PRD_ID, '+
            V_SQL+
            'FROM '+
            '(    SELECT 1 AS TIME_ID, M.PRD_ID,'+ V_MR_SUM +' FROM PMART.BASIC_MFACT_MR M , (SELECT DISTINCT L_DAY_ID,L_MONTH_ID FROM PMART.YMWD_TIME_W2) B '+
            ...
            '     WHERE A.TIME_ID=B.L_DAY_ID AND B.L_MONTH_ID IN('+ P_L_DAY_ID +') AND Y.ORG_ID=-1  '+
            ') U '+
            'WHERE M.TIME_ID=U.TIME_ID ';
   END IF;
   SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSM1_2_SUM_FUNC  AS('+
        V_STRING +
