...
SP:BEGIN
	...
	END IF;
    CALL PMART.P_DROP_TABLE ('#VT_FUNC_FACT_PRD_DIM_REF');
    SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_FUNC_FACT_PRD_DIM_REF ( '
    ...
    SET SQLSTR = 'INSERT INTO #VT_FUNC_FACT_PRD_DIM_REF(PARENT_PRD_ID,PRD_ID,FM_NAME,ALLOW_DRILL_DOWN,PRD_LEVEL,DRILL_PRD_LEVEL,DRILL_PRD_ID) '
	                     +'SELECT * FROM( ';
	 IF   P_REF_TABLE <> '' THEN 
          SET REFLIST = '  AND  ( PRD_ID IN ('  +  P_REF_TABLE  +   ' )  ' +
                                 '      OR  PRD_ID IN (SELECT DISTINCT KND_ID FROM PMART.PRD_GRP WHERE GRP_ID IN ('  +  P_REF_TABLE  +   ' )) ' +
                                 '      OR  PARENT_PRD_ID IN ('  +  P_REF_TABLE  +   ' )  ' + 
                                 '    )    '	 ;
	ELSE 	  
		  SET REFLIST = '' ;
	END IF ;
