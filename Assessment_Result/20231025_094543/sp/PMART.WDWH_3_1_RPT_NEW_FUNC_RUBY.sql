REPLACE PROCEDURE PMART.WDWH_3_1_RPT_NEW_FUNC_RUBY
(
  ...
  ;
   ...
   IF (I_ORG_TYPE = '-1') THEN
    SET V_DIM_SQL = V_DIM_SQL + '(
                    ...
                    UNION ALL
                    SELECT DISTINCT  0 AS ORG_LEVEL, PDEPT_ID AS ORG_ID FROM PMART.LAST_ORG_DIM WHERE TOT_ID = ' + I_ORG_ID + '
                   ) B';
   END IF;
   IF (I_ORG_TYPE = '0') THEN
    SET V_DIM_SQL = V_DIM_SQL + '(
                    SELECT DISTINCT -1 AS ORG_LEVEL, TOT_ID    AS ORG_ID FROM PMART.LAST_ORG_DIM
                    UNION ALL
					 SELECT DISTINCT  0 AS ORG_LEVEL, PDEPT_ID   AS ORG_ID FROM PMART.LAST_ORG_DIM WHERE PDEPT_ID = ' + I_ORG_ID + '
                    UNION ALL
                    SELECT DISTINCT  1 AS ORG_LEVEL, DEPT_ID   AS ORG_ID FROM PMART.LAST_ORG_DIM WHERE PDEPT_ID = ' + I_ORG_ID + '                   
                 ) B';
   END IF;
   IF (I_ORG_TYPE = '1') THEN
    SET V_DIM_SQL = V_DIM_SQL + '(
                    SELECT DISTINCT -1 AS ORG_LEVEL, TOT_ID    AS ORG_ID FROM PMART.LAST_ORG_DIM					
                     UNION ALL					
                    SELECT DISTINCT  0 AS ORG_LEVEL, PDEPT_ID   AS ORG_ID FROM PMART.LAST_ORG_DIM WHERE DEPT_ID = ' + I_ORG_ID + '
					UNION ALL					
   ...
   IF (I_ORG_TYPE = '-1') THEN
    SET V_REMD_SQL = V_REMD_SQL + ' SELECT ORG_ID AS MAP_ORG_ID, L_DAY_ID AS MAP_DAY_ID,
                                      ...
                                      WHERE A.ORG_ID IN (SELECT DISTINCT PDEPT_ID FROM PMART.LAST_ORG_DIM WHERE TOT_ID=' + I_ORG_ID + ')
                                        AND A.L_DAY_ID >= ' + V_LAST_14 + ' AND A.L_DAY_ID <= ' + I_DAY_ID + '';
   END IF;
   IF (I_ORG_TYPE = '0') THEN
    SET V_REMD_SQL = V_REMD_SQL +
	                                  '  SELECT ORG_ID AS MAP_ORG_ID, L_DAY_ID AS MAP_DAY_ID,
                                             ' + V_REMD_WHERE + '
                                       FROM PMART.REMD_FACT_SUM A
                                      WHERE A.ORG_ID IN (SELECT DISTINCT PDEPT_ID FROM PMART.LAST_ORG_DIM WHERE PDEPT_ID=' + I_ORG_ID + ')
                                        AND A.L_DAY_ID >= ' + V_LAST_14 + ' AND A.L_DAY_ID <= ' + I_DAY_ID + '
										UNION ALL
	                                   SELECT ORG_ID AS MAP_ORG_ID, L_DAY_ID AS MAP_DAY_ID,
                                             ' + V_REMD_WHERE + '
                                       FROM PMART.REMD_FACT_SUM A
                                      WHERE A.ORG_ID IN (SELECT DISTINCT DEPT_ID FROM PMART.LAST_ORG_DIM WHERE PDEPT_ID=' + I_ORG_ID + ')
                                        AND A.L_DAY_ID >= ' + V_LAST_14 + ' AND A.L_DAY_ID <= ' + I_DAY_ID + '';
   END IF;
   IF (I_ORG_TYPE = '1') THEN
    SET V_REMD_SQL = V_REMD_SQL + '
									 SELECT ORG_ID AS MAP_ORG_ID, L_DAY_ID AS MAP_DAY_ID,
                                             ' + V_REMD_WHERE + '
                                       FROM PMART.REMD_FACT_SUM A
                                      WHERE A.ORG_ID IN (SELECT DISTINCT PDEPT_ID FROM PMART.LAST_ORG_DIM WHERE DEPT_ID=' + I_ORG_ID + '
									                                                   UNION ALL
									                                                  SELECT DISTINCT DEPT_ID FROM PMART.LAST_ORG_DIM WHERE DEPT_ID=' + I_ORG_ID + ')
                                        AND A.L_DAY_ID >= ' + V_LAST_14 + ' AND A.L_DAY_ID <= ' + I_DAY_ID + '
                                      UNION ALL
                                     SELECT ORG_ID AS MAP_ORG_ID, L_DAY_ID AS MAP_DAY_ID,
                                             ' + V_REMD_WHERE + '
    ...
    SET V_REMD_SQL = V_REMD_SQL + ' SELECT ORG_ID AS MAP_ORG_ID, L_DAY_ID AS MAP_DAY_ID,
                                     ...
                                     SELECT A.OSTORE_ID AS MAP_ORG_ID, L_DAY_ID AS MAP_DAY_ID,
                                      ...
                                      WHERE A.OSTORE_ID IN (SELECT OSTORE_ID FROM PMART.LAST_ORG_DIM WHERE RESPON_ID=' + I_ORG_ID + ')
                                        AND A.L_DAY_ID >= ' + V_LAST_14 + ' AND A.L_DAY_ID <= ' + I_DAY_ID + '';
   END IF;   
     INSERT INTO  PMART.T1(F1, F2) VALUES (20200820, V_DIM_SQL);
   INSERT INTO  PMART.T1(F1, F2) VALUES (202008201, V_REMD_SQL);
     CALL PMART.P_DROP_TABLE ('#VT_WDWH_3_1_RPT_NEW_FUNC');
      SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_WDWH_3_1_RPT_NEW_FUNC AS         ' +
               ...
               INSERT INTO  PMART.T1(F1, F2) VALUES (202008202, SQLSTR);      
