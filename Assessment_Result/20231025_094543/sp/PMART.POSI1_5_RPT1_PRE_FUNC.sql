REPLACE PROCEDURE PMART.POSI1_5_RPT1_PRE_FUNC(P_L_DAY_ID VARCHAR(400),P_RPT VARCHAR(400),P_UNIT VARCHAR(400))
SQL SECURITY INVOKER
SP:BEGIN
   DECLARE SQLSTR  VARCHAR(8000); 
   DECLARE V_SELECT  VARCHAR(4000); 
   CALL PMART.P_DROP_TABLE ('#VT_POSI1_5_RPT1_PRE_FUNC');      
   SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_5_RPT1_PRE_FUNC  
                (TIME_ID NUMBER , PRD_ID NUMBER ,RESPON_ID NUMBER ,P_REAL_SALES_PSD NUMBER ,P_INPRD_RATE NUMBER ,P_REAL_SALES_AMT NUMBER) 
                NO PRIMARY INDEX ON COMMIT PRESERVE ROWS;';  
   EXECUTE IMMEDIATE SQLSTR;
   IF (P_RPT='RPT1_1' AND P_UNIT='GRP' ) THEN
      SET SQLSTR ='INSERT INTO #VT_POSI1_5_RPT1_PRE_FUNC(TIME_ID,PRD_ID,RESPON_ID,P_REAL_SALES_PSD,P_INPRD_RATE,P_REAL_SALES_AMT) '+
   ...
   IF (P_RPT='RPT1_3' AND P_UNIT='GRP' ) THEN
      SET SQLSTR ='INSERT INTO #VT_POSI1_5_RPT1_PRE_FUNC(TIME_ID,PRD_ID,RESPON_ID,P_REAL_SALES_PSD,P_INPRD_RATE,P_REAL_SALES_AMT) '+
        ...
        '  CASE WHEN (T.SL2_0103R) > 0 THEN (T.SL2_0103R-T.CS_0103R)/(CAST(T.SL2_0103R AS DECIMAL(16,6)))*100 ELSE 0 END , '+
        '  (T.SALES_AMT-T.DIS_AMT-T.SUB_AMT) '+
        '  FROM (SELECT * FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1) A LEFT OUTER JOIN PMART.BASIC_MFACT_BUDGET T '+
   ...
   IF (P_RPT='RPT2_1' AND P_UNIT='GRP' ) THEN
      SET SQLSTR ='INSERT INTO #VT_POSI1_5_RPT1_PRE_FUNC(TIME_ID,PRD_ID,RESPON_ID,P_REAL_SALES_PSD,P_INPRD_RATE,P_REAL_SALES_AMT) '+
        ...
        '  (T.SALES_AMT-T.DIS_AMT-T.SUB_AMT) '+        
        '  FROM PMART.BASIC_MFACT_BUDGET T  '+
        ' WHERE T.TIME_ID = FLOOR(TO_NUMBER(TO_CHAR(DateAdd(MONTH,''YYYYMM''),TO_DATE('''+ P_L_DAY_ID +'''),''YYYYMM''))/100 ) ';
   END IF;
   ...
   IF (P_RPT='RPT1_1' AND P_UNIT='BIGKND') THEN
      SET SQLSTR ='INSERT INTO #VT_POSI1_5_RPT1_PRE_FUNC(TIME_ID,PRD_ID,RESPON_ID,P_REAL_SALES_PSD,P_INPRD_RATE,P_REAL_SALES_AMT) '+
        ...
        '  CASE WHEN SUM(T.SL2_0103R) > 0 THEN SUM(T.SL2_0103R-T.CS_0103R)/SUM(CAST(T.SL2_0103R AS DECIMAL(16,6)))*100 ELSE 0 END , '+
        '  SUM(T.SALES_AMT-T.DIS_AMT-T.SUB_AMT) '+
        '  FROM PMART.BASIC_MFACT_BUDGET T INNER JOIN (SELECT DISTINCT GRP_ID ,KND_ID FROM PMART.PRD_DIM) A '+
        '    ON T.PRD_ID = A.GRP_ID '+
   ...
   IF (P_RPT='RPT2_1' AND P_UNIT='BRANCH') THEN
      SET SQLSTR ='INSERT INTO #VT_POSI1_5_RPT1_PRE_FUNC(TIME_ID,PRD_ID,RESPON_ID,P_REAL_SALES_PSD,P_INPRD_RATE,P_REAL_SALES_AMT) '+
        ...
        '  FROM PMART.BASIC_MFACT_BUDGET T INNER JOIN (SELECT * FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1) B '+
        '    ON T.PRD_ID = B.GRP_ID '+
        ' WHERE TIME_ID =  FLOOR(TO_NUMBER(TO_CHAR(DateAdd(MONTH,''YYYYMM''),TO_DATE('''+ P_L_DAY_ID +'''),''YYYYMM''))/100 ) '+        
        ' GROUP BY T.TIME_ID,B.BRANCH_ID ';             
   END IF;
