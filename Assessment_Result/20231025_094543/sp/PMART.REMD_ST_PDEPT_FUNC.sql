REPLACE PROCEDURE PMART.REMD_ST_PDEPT_FUNC(P_DAY_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_ST_PDEPT_FUNC'); 
  CALL PMART.REMD_S1_PDEPT_FUNC(P_DAY_ID);
  CALL PMART.REMD_S2_PDEPT_FUNC(P_DAY_ID);
  CALL PMART.REMD_S3_PDEPT_FUNC(P_DAY_ID);
  CALL PMART.REMD_S4_PDEPT_FUNC(P_DAY_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_ST_PDEPT_FUNC AS('+            
          ' SELECT '+
          ' -1 AS TOT_ID, '+
          ' A.ORG_ID AS ORG_ID, '+
		  ...
		  'CAST(A.BUDGET_DEPT_TOT_PLAN_STNUM AS NUMBER) AS BUDGET_DEPT_TOT_PLAN_STNUM, '+
		  'CAST(A.BUDGET_PDEPT_AVG_AMT AS NUMBER) AS BUDGET_PDEPT_AVG_AMT, '+
	      'CAST(A.BUDGET_PDEPT_TOT_AMT AS NUMBER) AS BUDGET_PDEPT_TOT_AMT, '+
		  'CAST(A.BUDGET_PDEPT_TOT_PLAN_STNUM AS NUMBER) AS BUDGET_PDEPT_TOT_PLAN_STNUM '+
          ' FROM '+
          ' #VT_REMD_S1_PDEPT_FUNC  A '+
          ' LEFT JOIN '+
          ' #VT_REMD_S2_PDEPT_FUNC  B '+
          '    ON (A.ORG_ID=B.ORG_ID) '+
          ' LEFT JOIN '+
          ' #VT_REMD_S3_PDEPT_FUNC  C '+
          '    ON (A.ORG_ID=C.ORG_ID) '+
          ' LEFT JOIN '+
          ' #VT_REMD_S4_PDEPT_FUNC  D '+
