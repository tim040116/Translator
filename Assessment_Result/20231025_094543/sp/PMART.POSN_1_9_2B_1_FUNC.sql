REPLACE PROCEDURE PMART.POSN_1_9_2B_1_FUNC
(
FP_TIME_TYPE CHAR,         
FP_TIME_LIST VARCHAR(400), 
FP_MMA_TYPE  NUMBER,       
FP_MMA_RANGE VARCHAR(400), 
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_POSN_1_9_2B_1_FUNC'); 
   CALL PMART.POSN_MFACT_1_9_1(FP_TIME_TYPE,FP_TIME_LIST,FP_PRD_LEVEL,FP_PRD_ID,1,FP_ORG_LEVEL,FP_ORG_LIST,FP_MMA_TYPE,FP_MMA_RANGE,FP_MMA_LIST,FP_PRD_TYPE,FP_AMT_TYPE);
   CALL PMART.POSN_STFACT_1_9_1(FP_TIME_TYPE,FP_TIME_LIST,FP_PRD_LEVEL,FP_PRD_ID,1,FP_ORG_LEVEL,FP_ORG_LIST,FP_MMA_TYPE,FP_MMA_RANGE,FP_MMA_LIST,FP_PRD_TYPE);
   CALL PMART.POSN_SFACT_1_9_1(FP_TIME_TYPE,FP_TIME_LIST,FP_ORG_LEVEL,FP_ORG_LIST,FP_MMA_TYPE,FP_MMA_RANGE,FP_MMA_LIST);
   SET V_SQL =
          ...
          'SUM(A.RETPRD_CNT) AS RETPRD_CNT, '+ 
          'DIVIDE_BY_ZERO(SUM(A.RETPRD_CNT), SUM(C.STNUM_STORE_NUM)) AS RETPRD_CNT_PSD, '+
          'DIVIDE_BY_ZERO(SUM(A.RETPRD_CNT), SUM(C.STNUM_STORE_NUM)) * 7 AS RETPRD_CNT_PSW, '+ 
          'SUM(A.TRANSPRD_CNT) AS TRANSPRD_CNT, '+ 
          'DIVIDE_BY_ZERO(SUM(A.TRANSPRD_CNT), SUM(C.STNUM_STORE_NUM)) AS TRANSPRD_CNT_PSD, '+ 
          'DIVIDE_BY_ZERO(SUM(A.TRANSPRD_CNT), SUM(C.STNUM_STORE_NUM)) * 7 AS TRANSPRD_CNT_PSW, '+ 
     ...
     SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSN_1_9_2B_1_FUNC  AS('+
          'SELECT '+
          'A.PRD_ID, '+ 
          'A.ORG_ID, '+ 
          V_SQL +
          ' FROM #VT_POSN_MFACT_1_9_1 A '+
          ' ,#VT_POSN_STFACT_1_9_1 B '+
          ' ,#VT_POSN_SFACT_1_9_1 C  '+
          ' WHERE A.TIME_ID=B.TIME_ID AND A.ORG_ID=B.ORG_ID AND A.PRD_ID=B.PRD_ID AND A.MMA_ID=B.MMA_ID  '+
          ' AND A.TIME_ID=C.TIME_ID AND A.ORG_ID=C.ORG_ID AND A.MMA_ID=C.MMA_ID GROUP BY A.PRD_ID,A.ORG_ID'+
