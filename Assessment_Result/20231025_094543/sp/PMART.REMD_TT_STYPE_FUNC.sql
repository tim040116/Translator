REPLACE PROCEDURE PMART.REMD_TT_STYPE_FUNC(P_DAY_ID NUMBER,P_ORG_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_TT_STYPE_FUNC'); 
  CALL PMART.REMD_T1_STYPE_FUNC(P_DAY_ID,P_ORG_ID);
  CALL PMART.REMD_T2_STYPE_FUNC(P_DAY_ID,P_ORG_ID);
  CALL PMART.REMD_T3_STYPE_FUNC(P_DAY_ID,P_ORG_ID);
  CALL PMART.REMD_T4_STYPE_FUNC(P_DAY_ID,P_ORG_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_TT_STYPE_FUNC AS('+            
                'SELECT '+
                '-1 AS TOT_ID, '+
                ...
                'FROM '+
                '#VT_REMD_T1_STYPE_FUNC A '+
                'LEFT JOIN '+
                '#VT_REMD_T2_STYPE_FUNC B '+
                '   ON (A.STYPE_ID=B.STYPE_ID) '+
                'LEFT JOIN '+
                ' #VT_REMD_T3_STYPE_FUNC C '+
                '   ON (A.STYPE_ID=C.STYPE_ID) '+
                'LEFT JOIN '+
                '#VT_REMD_T4_STYPE_FUNC D '+
