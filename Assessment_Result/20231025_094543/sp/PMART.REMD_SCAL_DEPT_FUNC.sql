REPLACE PROCEDURE PMART.REMD_SCAL_DEPT_FUNC(P_DAY_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(5000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_SCAL_DEPT_FUNC_1');
  CALL PMART.P_DROP_TABLE ('#VT_REMD_SCAL_DEPT_FUNC');
  CALL PMART.REMD_ST_DEPT_FUNC(P_DAY_ID);
  SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_REMD_SCAL_DEPT_FUNC_1  
   (
  ...
  INSERT INTO #VT_REMD_SCAL_DEPT_FUNC_1(R14  ,R15 ) SELECT '僅課別以下顯示' ,NULL ; 
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_SCAL_DEPT_FUNC AS('+
                'SELECT '+
                'TOT_ID, '+
                'ORG_ID, '+
		...
		' ( SELECT R14 FROM #VT_REMD_SCAL_DEPT_FUNC_1 ) AS R14,'+
		' PMART.GET_WEATHER_NM(0) AS R14_NAME, '+
		' ( SELECT R15 FROM #VT_REMD_SCAL_DEPT_FUNC_1 ) AS R15,'+               
