REPLACE PROCEDURE PMART.B2BIGA_1_2_FUNC
(
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_B2BIGA_1_2_FUNC'); 
	...
	END IF;
   SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_B2BIGA_1_2_FUNC  AS( '
        + ' SELECT C.FM_NAME AS CMNM, '
		+ ' B.CMNO AS CMNO, '
		+ ' B.COUNT1 AS COUNT1 '
		+ ' FROM ( '
		+ ' SELECT A.CMNO,SUM(COUNT1) AS COUNT1 '
		+ ' FROM '+ V_TABLE_NAME +' A '
		+ ' WHERE '+ V_TIME_STR +' ' 
		+ ' AND '+ V_PRD_ID_STR +' '
		+ ' GROUP BY A.CMNO) B, '
