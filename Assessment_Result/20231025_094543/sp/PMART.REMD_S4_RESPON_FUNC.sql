REPLACE PROCEDURE PMART.REMD_S4_RESPON_FUNC(P_DAY_ID NUMBER,P_BRANCH_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_S4_RESPON_FUNC'); 
  ...
  LOCKING PMART.LAST_ORG_DIM FOR ACCESS;
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_S4_RESPON_FUNC AS('+
          ' SELECT '+
          ' B.ORG_ID AS ORG_ID, '+
          ...
          ' ON 1 = 1 '+
          ' LEFT OUTER JOIN '+
          ' (SELECT C1.*,C2.RESPON_ID AS ORG_ID '+
