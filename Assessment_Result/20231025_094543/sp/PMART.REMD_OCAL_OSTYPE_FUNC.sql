REPLACE PROCEDURE PMART.REMD_OCAL_OSTYPE_FUNC
(P_DAY_ID NUMBER,P_OSTYPE_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(10000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_OCAL_OSTYPE_FUNC'); 
  CALL PMART.REMD_OT_OSTYPE_FUNC(P_DAY_ID,P_OSTYPE_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_OCAL_OSTYPE_FUNC AS('+            
              'SELECT '+
                  'TOT_ID, '+
                  'OSTORE_ID, '+
                 ...
                 '         DECODE(T1.EX_TOT_PLAN_STNUM_LAST_YEAR,0,NULL,EX_TOT_PLAN_STNUM_LAST_YEAR))=0 THEN 0 ELSE    '+
                  ...
                  'FROM #VT_REMD_OT_OSTYPE_FUNC T1 '+
