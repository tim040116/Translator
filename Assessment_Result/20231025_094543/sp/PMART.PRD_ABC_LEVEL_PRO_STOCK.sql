REPLACE PROCEDURE PMART.PRD_ABC_LEVEL_PRO_STOCK(P_YEAR_ID NUMBER)
...
DELETE FROM PTEMP.TP_PRD_SAL_STOCK;
     ...
     SELECT S.TIME_ID, S.PRD_ID, D.GRP_NO,NVL(D.PRD_SLUNT,0) ,SUM(NVL(S.SALCNT,0))
                       ,SUM(NVL(CASE WHEN TO_NUMBER(DECODE(C.REAL_CODE ,NULL,'0' , C.REAL_CODE)) <> 10 THEN (D.PRD_SLUNT-D.PRD_CSUNT*1.05)*S.SALCNT       
													 WHEN TO_NUMBER(DECODE(C.REAL_CODE ,NULL,'0' , C.REAL_CODE)) =  10 THEN S.SALAMT - (S.SALAMT * CASE WHEN D.PRD_SLUNT = 0 THEN 0 ELSE (D.PRD_CSUNT/D.PRD_SLUNT) END ) 
                                           END,0))/SUM(STCNT) AS NET_PROFIT             
          FROM PMART.PRD_DIM D, PDATA.PBMCMDT C, (SELECT * FROM PTEMP.TP_BASIC_MFACT_STOCK_SAL WHERE TIME_ID=P_YEAR_ID)S
      WHERE D.PRD_ID = S.PRD_ID AND D.PRD_ID = C.FM_CODE
GROUP BY  S.TIME_ID, S.PRD_ID, D.GRP_NO,NVL(D.PRD_SLUNT,0);
