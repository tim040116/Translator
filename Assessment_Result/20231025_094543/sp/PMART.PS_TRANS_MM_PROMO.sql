REPLACE   PROCEDURE PMART.PS_TRANS_MM_PROMO
(
)
SP:BEGIN
	...
	SET P_TX_EDT = '''' + (SELECT TX_EDT + ' 23:59:59' AS TX_EDX FROM PDATA.PRDREL_SET WHERE  JOB_ID = P_JOB_ID) + '''';
 	CALL PMART.P_DROP_TABLE ('#VT_PS_TRANS_MM_PROMO');
SET SQLSTR = ' 
SELECT * FROM PDATA.TRANS_MM_PROMO
WHERE  TX_DTTM BETWEEN  '+ P_TX_SDT + ' AND '+P_TX_EDT+'   
'
; 
   SET SQLCREATE = 
   'CREATE MULTISET VOLATILE TABLE #VT_PS_TRANS_MM_PROMO AS( '
   +SQLSTR 
