REPLACE PROCEDURE PMART.FUNC_FACT_MMK_ENT
(
   IN P_RPTID SMALLINT,
...
CALL PMART.P_DROP_TABLE ('#VT_FACT_MMK_ENT_TEMP');
CALL PMART.P_DROP_TABLE ('#VT_FACT_MMK_ENT');
IF P_RPTID = 1  THEN
	SET ID_VAL = P_ORGID;
	IF P_ORGTYPE = '0' THEN
...
ELSE
	...
	END IF;
	IF P_MMKTYPE = '2' THEN
		CALL PMART.FUNC_FACT_MMK_DIM(1,'2','03', SUBSTRING(TRIM(P_MMKID),1,3)); 
		SET MMKWHERE = 'SELECT MMK_VAL GROUP_BY_ID, MMK_VAL MMK_ID '
								     + ' FROM #VT_FUNC_FACT_MMK_DIM '
	                                 + ' WHERE MMK_VAL <> ''-1'' '
									 + ' AND MMK_VAL = ''' + P_MMKID + '''';
	ELSE
		CALL PMART.FUNC_FACT_MMK_DIM(1,'2','03', P_MMKID);
		SET MMKWHERE = 'SELECT MMK_VAL GROUP_BY_ID, MMK_VAL MMK_ID '
								     + ' FROM #VT_FUNC_FACT_MMK_DIM '
                                     + ' WHERE MMK_VAL <> ''-1'' ';
	END IF;	 
...
END IF;
				...
				SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_FACT_MMK_ENT AS( '
