REPLACE PROCEDURE PMART.POSI1_2_FUNC_RUBY(P_L_DAY_ID VARCHAR(400),P_DAY_TYPE VARCHAR(1), P_LEVEL VARCHAR(1), P_PRD_ID VARCHAR(400) )
...
SP:BEGIN
   ...
   SET V_SELECT  =' SUM(T.SALES_AMT) AS SALES_AMT, '+
		  ...
		  ' SUM(T.BUDGET_AMT) AS BUDGET_AMT, '+
                  ...
                  ' CASE WHEN SUM(TL.SALES_AMT) = 0 THEN 0 ELSE SUM(CAST(T.SALES_AMT AS DECIMAL(16,6))-T.DIS_AMT-T.SUB_AMT)/SUM(TL.SALES_AMT-TL.DIS_AMT-TL.SUB_AMT)*100 END AS LSALES_RATE, '+
                  ' CASE WHEN SUM(TL.SALES_AMT) = 0 OR MAX(T.UPLOAD_STNUM)=0 OR MAX(TL.UPLOAD_STNUM)=0 THEN 0 ELSE (SUM(T.SALES_AMT-T.DIS_AMT-T.SUB_AMT)/MAX(CAST(T.UPLOAD_STNUM AS DECIMAL(16,6))))/(SUM(TL.SALES_AMT-TL.DIS_AMT-TL.SUB_AMT)/MAX(CAST(TL.UPLOAD_STNUM AS DECIMAL(16,6))))*100 END AS L_YEAR_PSD_DIFF ';
   ...
   CASE P_LEVEL
         ...
         SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_2_FUNC  AS('+
                     ...
                     '  CASE WHEN (TL.SALES_AMT) = 0 THEN 0 ELSE (CAST(T.SALES_AMT AS DECIMAL(16,6))-T.DIS_AMT-T.SUB_AMT)/(TL.SALES_AMT-TL.DIS_AMT-TL.SUB_AMT)*100 END AS LSALES_RATE, '+
                     '  CASE WHEN (TL.SALES_AMT) = 0 OR T.UPLOAD_STNUM=0 OR TL.UPLOAD_STNUM=0 THEN 0 ELSE ((T.SALES_AMT-T.DIS_AMT-T.SUB_AMT)/(CAST(T.UPLOAD_STNUM AS DECIMAL(16,6))))/((TL.SALES_AMT-TL.DIS_AMT-TL.SUB_AMT)/(CAST(TL.UPLOAD_STNUM AS DECIMAL(16,6))))*100 END AS L_YEAR_PSD_DIFF '+
         ...
         SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_2_FUNC  AS('+
                     ...
                     '     ON T.TIME_ID - '+ LDAY +' = TL.TIME_ID '+
                     '    AND T.PRD_ID  = TL.PRD_ID '+
                     '                                    INNER JOIN (SELECT DISTINCT GRP_ID ,KND_ID FROM PMART.PRD_DIM) A '+
         ...
         SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_2_FUNC  AS('+  
                     ...
                     '     ON T.TIME_ID - '+ LDAY +' = TL.TIME_ID '+
                     '    AND T.PRD_ID  = TL.PRD_ID '+                     
                     '                                    INNER JOIN (SELECT DISTINCT RESPON_ID,GRP_ID FROM PMART.ORG_DIM_POSI1 WHERE SEQNO = 1 ) B '+
         ...
         SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_2_FUNC  AS('+
                     ...
                     '     ON T.TIME_ID - '+ LDAY +' = TL.TIME_ID '+
                     '    AND T.PRD_ID  = TL.PRD_ID '+
                     '                                    INNER JOIN (SELECT DISTINCT BRANCH_ID,GRP_ID FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1 ) B '+
                     '     ON T.PRD_ID  = B.GRP_ID '+ 
                     '  WHERE T.TIME_ID IN('+ P_L_DAY_ID +') '+
                     '    AND T.PRD_ID  IN('+ P_PRD_ID +') '+                     
         ...
         SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_2_FUNC  AS('+
                     ...
                     '     ON T.TIME_ID - '+ LDAY +' = TL.TIME_ID '+
                     '    AND T.PRD_ID  = TL.PRD_ID '+
                     '                                    INNER JOIN (SELECT DISTINCT DEPT_ID,GRP_ID FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1 ) B '+
                     '     ON T.PRD_ID  = B.GRP_ID '+                  
                     '  WHERE T.TIME_ID IN('+ P_L_DAY_ID +') '+
                     '    AND T.PRD_ID  IN('+ P_PRD_ID +') '+
         ...
         SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_2_FUNC  AS('+
                     ...
                     '     ON T.TIME_ID - '+ LDAY +' = TL.TIME_ID '+
                     '    AND T.PRD_ID  = TL.PRD_ID '+
                     '                                   INNER JOIN (SELECT DISTINCT TOT_ID,GRP_ID FROM PMART.ORG_DIM_POSI1 WHERE SEQNO=1 ) B '+
                     '     ON T.PRD_ID  = B.GRP_ID '+
                     '  WHERE T.TIME_ID IN('+ P_L_DAY_ID +') '+
                     '    AND T.PRD_ID  IN('+ P_PRD_ID +') '+                    
	...
	SET SQLSTR ='';                
      END CASE ;      
	INSERT INTO PMART.VT_DEBUG_LOG('POSI1_2_FUNC:', 'SQLSTR:'+SQLSTR );
