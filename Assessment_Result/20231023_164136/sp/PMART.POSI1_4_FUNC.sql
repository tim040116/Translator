REPLACE PROCEDURE PMART.POSI1_4_FUNC (P_L_DAY_ID VARCHAR(400), P_LEVEL VARCHAR(2), P_PRD_ID VARCHAR(400) )
SQL SECURITY INVOKER
SP:BEGIN
   DECLARE SQLSTR  VARCHAR(4000); 	
   DECLARE V_TABLE_NAME VARCHAR(30);
   CALL PMART.P_DROP_TABLE ('#VT_POSI1_4_FUNC'); 
   ...
   SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_4_FUNC  AS('+
                     'SELECT '+
                     'M.TIME_ID, '+
                     'M.PRD_ID, '+
                     'M.SALES_AMT, '+
                     ...
                     'M.BUDGET_AMT AS BUDGET_AMT , '+
                     'M.BUDGET_AMT2 AS BUDGET_AMT2, '+
                     'M.BUDGET_AMT_PSD, '+
                     'M.BUDGET_AMT_PSD2, '+
                     'M.BUDGET_SALES_RATE , '+
