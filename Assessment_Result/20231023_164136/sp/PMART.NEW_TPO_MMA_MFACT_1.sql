REPLACE PROCEDURE PMART.NEW_TPO_MMA_MFACT_1
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_NEW_TPO_MMA_MFACT_1'); 
   CASE
      ...
      ELSE
             SET V_TABLE_NAME = 'PMART.BASIC_MFACT_SHOP';
   END CASE;
   SET SQLSTR1 = 'CREATE MULTISET VOLATILE TABLE #VT_NEW_TPO_MMA_MFACT_1  AS('+
     'SELECT '+
         'F1.MMA_ID AS MMA_ID, '+
         'F1.PRD_ID AS PRD_ID, '+
	 ...
	 'CAST(F2.SALES_DISSUB_AMT AS DECIMAL(18,6))/1000 AS LSALES_DISSUB_EXTEND_AMT, ';
         ...
         '( '+
         'SELECT  '+
         'DENSE_RANK() OVER(ORDER BY TIME_ID,MMA_ID,PRD_ID) AS JOIN_KEY, '+
         'MMA_ID AS MMA_ID, '+
         'PRD_ID AS PRD_ID, '+
         'TIME_ID AS TIME_ID, '+
