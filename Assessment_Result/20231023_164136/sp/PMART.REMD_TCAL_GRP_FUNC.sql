REPLACE PROCEDURE PMART.REMD_TCAL_GRP_FUNC
(P_L_DAY_ID NUMBER,P_ORG_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_TCAL_GRP_FUNC'); 
  CALL PMART.REMD_TT_GRP_FUNC(P_L_DAY_ID,P_ORG_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_TCAL_GRP_FUNC AS('+            
                'SELECT '+
                'TOT_ID, '+
                'STYPE_ID, '+
                ...
                'ACCU_AMT*0.9561/DECODE(ACCU_BUDGET,0,NULL,ACCU_BUDGET)*100 AS R6, '+
                '(REMD_ADD_TKSL('+P_L_DAY_ID+',''Y'',EX_ACCU_AMT,EX_ACCU_TKSL)/DECODE(EX_ACCU_PLAN_STNUM,0,NULL,EX_ACCU_PLAN_STNUM))/ '+
                '   (REMD_ADD_TKSL('+P_L_DAY_ID+',''Y'',EX_TOT_AMT_LAST_YEAR,EX_TOT_TKSL_LAST_YEAR)/ '+
                '   DECODE(EX_TOT_PLAN_STNUM_LAST_YEAR,0,NULL,EX_TOT_PLAN_STNUM_LAST_YEAR))*100 '+
                'AS R7, '+
                '(REMD_ADD_TKSL('+P_L_DAY_ID+',''M'',ACCU_AMT,ACCU_TKSL)/DECODE(ACCU_PLAN_STNUM,0,NULL,ACCU_PLAN_STNUM))/ '+
                '   DECODE(REMD_ADD_TKSL('+P_L_DAY_ID+',''M'',TOT_AMT_LAST_MONTH,TOT_TKSL_LAST_MONTH)/ '+
                '   DECODE(TOT_PLAN_STNUM_LAST_MONTH,0,NULL,TOT_PLAN_STNUM_LAST_MONTH), '+
                '   0,NULL, REMD_ADD_TKSL('+P_L_DAY_ID+',''M'',TOT_AMT_LAST_MONTH,TOT_TKSL_LAST_MONTH)/ '+
                ...
                '(REMD_ADD_TKSL('+P_L_DAY_ID+',''Y'',ACCU_AMT,ACCU_TKSL)/DECODE(ACCU_CUST_NUM,0,NULL,ACCU_CUST_NUM))- '+
                '   (REMD_ADD_TKSL('+P_L_DAY_ID+',''Y'',EX_TOT_AMT_LAST_YEAR,EX_TOT_TKSL_LAST_YEAR)/ '+
