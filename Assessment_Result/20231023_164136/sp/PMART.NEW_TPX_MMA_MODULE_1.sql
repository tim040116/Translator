CREATE PROCEDURE PMART.NEW_TPX_MMA_MODULE_1
...
DECLARE SQLSTR  VARCHAR(64000);
   CALL PMART.P_DROP_TABLE ('#VT_NEW_TPX_MMA_MODULE_1'); 
   ...
   SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_NEW_TPX_MMA_MODULE_1  AS('+
                    'SELECT '+
                    'A.TIME_ID AS TIME_ID, '+
                    'A.PRD_ID AS PRD_ID, '+
                    'A.MMA_ID AS MMA_ID, '+
                    'B.ORDER_STORE_NUM AS ORDER_EXTEND_STORE_NUM, '+
                    'CAST(B.ORDER_STORE_NUM AS DECIMAL(18,6)) /DECODE(B.MAST_STORE_NUM,0,NULL,B.MAST_STORE_NUM)*100 AS ORDER_EXTEND_STORE_RATE, '+
                    'B.SALES_STORE_NUM AS SALES_EXTEND_STORE_NUM, '+
                    'CAST(B.SALES_STORE_NUM AS DECIMAL(18,6)) /DECODE(B.MAST_STORE_NUM,0,NULL,B.MAST_STORE_NUM)*100 AS SALES_EXTEND_STORE_RATE, '+
                    ...
                    'CAST(B.SALES_ORDER_STORE_NUM AS DECIMAL(18,6))/DECODE(B.MAST_STORE_NUM,0,NULL,B.MAST_STORE_NUM)*100 AS LEAD_EXTEND_STORE_RATE, '+
                    ...
                    'A.ITMTRANSFER_EXTEND_PRE_NUM AS ITMTRANSFER_EXTEND_PRE_NUM '+
                     'FROM '+
                     '  #VT_NEW_TPX_MMA_MFACT_1 A LEFT JOIN  '+
                     '  #VT_NEW_TPX_MMA_STFACT B  '+
