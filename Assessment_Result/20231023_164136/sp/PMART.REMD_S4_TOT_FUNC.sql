REPLACE PROCEDURE PMART.REMD_S4_TOT_FUNC(P_DAY_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_S4_TOT_FUNC'); 
  ...
  LOCKING PMART.LAST_ORG_DIM FOR ACCESS;  
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_S4_TOT_FUNC AS('+            
          'SELECT '+
          '-1 AS ORG_ID, '+
          ...
          ' INNER JOIN '+          
          '(SELECT ORG_ID ,MASK FROM PMART.LAST_ORG_DIM_MASK WHERE P_ORG_ID=-2) B '+
          ' ON 1 = 1 '+
          ' LEFT OUTER JOIN '+
          ' (SELECT C1.*,C2.TOT_ID AS ORG_ID '+
