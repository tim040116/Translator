REPLACE  PROCEDURE PDATA.FAMI_TRAFFIC_0200
(
)
SP:BEGIN
DECLARE PBATCH_DTTM INTEGER;
SELECT TO_NUMBER(TO_CHAR(BATCH_DTTM, 'YYYYMMDD')) INTO PBATCH_DTTM
FROM PSTAGE.DWH_CTL WHERE TABLENAME = 'FAMI_TRAFFIC';
CALL PDATA.P_DROP_TABLE('PTEMP.TP0_FAMI_TRAFFIC');
CREATE SET TABLE PTEMP.TP0_FAMI_TRAFFIC  ,
,
,
,
...
CALL PDATA.P_DROP_TABLE('PTEMP.TP1_FAMI_TRAFFIC');
CREATE MULTISET TABLE PTEMP.TP1_FAMI_TRAFFIC  ,
,
,
,
     DEFAULT MERGEBLOCKRATIO
...
INSERT INTO PTEMP.TP1_FAMI_TRAFFIC
SELECT TO_CHAR(ORDER_DATE,'YYYYMMDD') AS L_DAY_ID,
  ...
  FROM PSTAGE.ST_FAMI_TRAFFIC FAMI
...
INSERT INTO PTEMP.TP0_FAMI_TRAFFIC
SELECT  L_DAY_ID , OSTORE_ID , MMK_ID , TRAN_NO ,  TICKET_COUNT ,
TOTAL_PRICE , ORDER_DATE , ITEM_NAME , STATUS , DATA_CNT
FROM PTEMP.TP1_FAMI_TRAFFIC;
FOR LOOPVAR AS CUR1 CURSOR FOR
SELECT DISTINCT L_DAY_ID FROM PTEMP.TP1_FAMI_TRAFFIC ORDER BY L_DAY_ID
DO 
INSERT INTO PTEMP.TP0_FAMI_TRAFFIC
SELECT B. L_DAY_ID,
               B.OSTORE_ID,
			   ...
			   B.STATUS, 
			   -1 AS DATA_CNT
   FROM PTEMP.TP1_FAMI_TRAFFIC A
   JOIN (SELECT * FROM PDATA.FAMI_TRAFFIC 
...
(TRIM(MMK_ID)+TRIM(YYY)+TRIM(SUBSTRING('0000',1,4-LENGTH(TO_CHAR(NEWSEQ))))+TRIM(NEWSEQ)) ITEM_ID,
ITEM_NAME, DATE
FROM (
...
FROM
(
SELECT DISTINCT '2' SYS_ID, '04' CLASS_ID, MMK_ID, ITEM_NAME FROM PTEMP.TP1_FAMI_TRAFFIC
) A
...
MERGE INTO PDATA.FAMI_TRAFFIC A
USING PTEMP.TP1_FAMI_TRAFFIC B
...
DROP TABLE PTEMP.TP1_FAMI_TRAFFIC;
END SP;