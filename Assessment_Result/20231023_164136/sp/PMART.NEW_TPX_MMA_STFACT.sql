REPLACE PROCEDURE PMART.NEW_TPX_MMA_STFACT
...
DECLARE V_TABLE_NAME VARCHAR(30);
   ...
   CASE
      WHEN FP_TIME_TYPE='D' AND FP_PRD_TYPE='P' AND FP_PRD_LEVEL=2 THEN  
            SET V_TABLE_NAME = 'PMART.BASIC_STFACT_DETAIL_SHOP';
      WHEN FP_TIME_TYPE='D' AND FP_PRD_TYPE='R' AND FP_PRD_LEVEL=1  THEN  
            SET V_TABLE_NAME = 'PMART.BASIC_STFACT_DETAIL_SHOP';
    ...
    END CASE;
    SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_NEW_TPX_MMA_STFACT  AS('+
					 ...
					 'BIT_EXTRACT(BIT_AND(BIT_OR(F1.SALES_STORE_NUM,F1.ORDER_STORE_NUM),Y.MASK)) AS SALES_ORDER_STORE_NUM, '+
                     ...
                     ') F1, '+
                     '( '+
                     'SELECT TIME_ID,MMA_ID,MAST_STORE_NUM FROM '+
					 ...
					 'WHERE '+
					 'F1.TIME_ID=F2.TIME_ID AND F1.MMA_ID = F2.MMA_ID '+
