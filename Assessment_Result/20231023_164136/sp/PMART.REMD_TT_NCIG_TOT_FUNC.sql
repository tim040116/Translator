REPLACE PROCEDURE PMART.REMD_TT_NCIG_TOT_FUNC(P_DAY_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_TT_NCIG_TOT_FUNC'); 
  CALL PMART.REMD_T1_NCIG_TOT_FUNC(P_DAY_ID);
  CALL PMART.REMD_T2_NCIG_TOT_FUNC(P_DAY_ID);
  CALL PMART.REMD_T3_NCIG_TOT_FUNC(P_DAY_ID);
  CALL PMART.REMD_T4_NCIG_TOT_FUNC(P_DAY_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_TT_NCIG_TOT_FUNC AS('+            
                'SELECT '+
                '-1 AS TOT_ID, '+
                '-1 AS ORG_ID, '+
                ...
                'A.EX_ACCU_TOT_AMT AS EX_ACCU_AMT, '+
                ...
                'FROM '+
                '#VT_REMD_T1_NCIG_TOT_FUNC A '+
                'LEFT JOIN '+
                '#VT_REMD_T2_NCIG_TOT_FUNC B '+
                '   ON (A.ORG_ID=B.ORG_ID) '+
                'LEFT JOIN '+
                '#VT_REMD_T3_NCIG_TOT_FUNC C '+
                '   ON (A.ORG_ID=C.ORG_ID) '+
                'LEFT JOIN '+
                '#VT_REMD_T4_NCIG_TOT_FUNC D '+
