REPLACE PROCEDURE PMART.OLD0401_FUNC_FACT_PAS
(
   IN P_RPTID SMALLINT,
...
SP:BEGIN
   ...
   END IF;
      IF   (P_ACT_CATEGORY = '1')  THEN  
        SET ACTWHERE = ' AND ACT_CATEGORY =1 ' ;
   ELSEIF (P_ACT_CATEGORY = '2') THEN
		 SET ACTWHERE = ' AND ACT_CATEGORY =2 ';
   ELSE  
		 SET ACTWHERE = ' AND ACT_CATEGORY =3 ';   
   END IF;
   IF P_RPTID = 1 THEN
	   SET BASETABLE = ' PMART.FACT_MEMBER_PAS_PRD ';
	   SET Y_DIM = ' B.ORG_ID ';
		IF P_PRDTYPE = -1 THEN
			SET SELECT_CALCNT = ' AND B.PRD_ID = -1 ';
		ELSE
			SET SELECT_CALCNT = ' AND B.PRD_ID = '+P_PRDID;
		END IF;
...
SET SQLSTR = ' CREATE MULTISET VOLATILE TABLE #VT_PRD AS( '
					  ...
					  +'             SELECT DISTINCT  CAST(TO_NUMBER(A.FM_CODE) AS VARCHAR(7)) AS PRD_ID,  '
     				  +'                                              CAST(TO_NUMBER(C.KND+C.GRP) AS VARCHAR(3)) AS GRP_ID, '
      				  +'                                              CAST(TO_NUMBER(C.KND) AS VARCHAR(2))AS KND_ID '
            		  ...
            		  +'													 (SELECT DISTINCT KIND_CODE,GROUP_CODE,GROUP_NAME FROM PDATA.PBMKGRP WHERE GROUP_CODE<>0)G'
