REPLACE PROCEDURE PMART.REMD_SCAL_BRANCH_FUNC(P_DAY_ID NUMBER,P_DEPT_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(5000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_SCAL_BRANCH_FUNC'); 
 CALL PMART.REMD_ST_BRANCH_FUNC(P_DAY_ID,P_DEPT_ID); 
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_SCAL_BRANCH_FUNC AS('+            
	       'SELECT '+
               'TOT_ID, '+
               'ORG_ID, '+
               ...
               'FROM #VT_REMD_ST_BRANCH_FUNC T1 '+
