REPLACE  PROCEDURE PMART.BI_1_3_1_FUNC(P_TIME_ID NUMBER,P_LEVEL NUMBER,P_ORG_ID NUMBER,P_PRD_ID VARCHAR(7),P_FILTER_TYPE NUMBER,P_PRD_VALUE NUMBER,P_AVGINPRD NUMBER
,P_C_RATE NUMBER,P_CP_RATE NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
   DECLARE SQLSTR VARCHAR(8000);
   DECLARE FILTER_STR VARCHAR(400);
   CALL PMART.P_DROP_TABLE ('#VT_BI_1_3_1_FUNC');          
   ...
   END IF;  
   CALL PMART.BI_1_3_1_S1_FUNC(P_TIME_ID,P_LEVEL,P_ORG_ID,P_PRD_ID,P_PRD_VALUE) ;
   CALL PMART.BI_1_3_1_S2_FUNC(P_TIME_ID,P_LEVEL,P_ORG_ID,P_PRD_ID,P_PRD_VALUE) ;
   CALL PMART.BI_1_3_1_ST_FUNC(P_LEVEL,P_ORG_ID) ;
   CALL PMART.BI_1_3_1_S3_FUNC(P_TIME_ID) ;
   SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_BI_1_3_1_FUNC  AS('+
               ...
               ' ,AA.RESPON_NM,AA.BRANCH_NM,AA.DEPT_NM,AA.PDEPT_NM,AA.PDEPT_ID,AA.DEPT_ID,AA.BRANCH_ID,AA.RESPON_ID' +
               ' FROM ( ' +
               ...
               'FROM #VT_BI_1_3_1_S1_FUNC A '+
               'LEFT OUTER JOIN '+
               '      #VT_BI_1_3_1_S2_FUNC B '+
               'ON (A.ORG_ID=B.ORG_ID AND A.PRD_ID=B.PRD_ID) '+
               'INNER JOIN '+
               '      #VT_BI_1_3_1_ST_FUNC S '+
               'ON (A.ORG_ID = S.STORE_ID) '+
               'INNER JOIN '+
               '      #VT_BI_1_3_1_S3_FUNC T '+
