REPLACE PROCEDURE PMART.BI_REMD_DEPT_FUNC(P_WEEK_ID INTEGER)
...
SP:BEGIN
   ...
   DECLARE REMD_CS CURSOR FOR REMD_SQL;
    CALL PMART.P_DROP_TABLE ('#VT_BI_REMD_DEPT_FUNC'); 
   SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_BI_REMD_DEPT_FUNC  
    (    
   ...
   SET SQLSTR = 'SELECT A.L_DAY_ID, A.ORG_ID AS ORG_ID, C.L_WEEK_ID AS L_WEEK_ID, '+
       ...
       '  FROM PMART.REMD_FACT_SUM A INNER JOIN (SELECT DISTINCT DEPT_ID FROM PMART.LAST_ORG_DIM WHERE TOT_ID=-1 ) B '+
   ...
   DO
	...
	FETCH REMD_CS INTO V_DAY_ID, V_ORG_ID, V_WEEK_ID, V_UPLOAD_STNUM, V_AMT, V_CUST_NUM, V_P_UPLOAD_STNUM, V_P_AMT, V_P_CUST_NUM;
		...
		IF P_ORG_ID <>  V_ORG_ID THEN
			IF P_ORG_ID <> 0 THEN 
				L2 :
				FOR CUR2 AS BI_REMD_DEPT_FUNC_C2 CURSOR FOR 				
				...
				INSERT INTO #VT_BI_REMD_DEPT_FUNC (L_DAY_ID, ORG_ID, L_WEEK_ID, AMT, CUST_NUM, CUST_AMT, P_AMT ,P_CUST_NUM) VALUES (V_DAY_ID_C2, V_ORG_ID_C2, V_WEEK_ID_C2, V_AMT_C2, V_CUST_NUM_C2, V_CUST_AMT_C2, V_P_AMT_C2, V_P_CUST_NUM_C2);
				END FOR L2;
			END IF;
	    ...
	    ELSE
		   ...
		   END IF;	  
		END IF ;   		
		INSERT INTO #VT_BI_REMD_DEPT_FUNC (L_DAY_ID, ORG_ID, L_WEEK_ID, AMT, CUST_NUM, CUST_AMT, P_AMT ,P_CUST_NUM) VALUES (V_DAY_ID_C1, V_ORG_ID_C1, V_WEEK_ID_C1, V_AMT_C1, V_CUST_NUM_C1, V_CUST_AMT_C1, V_P_AMT_C1, V_P_CUST_NUM_C1);
	END;
   END WHILE L1;
	L3:
	FOR CUR3 AS BI_REMD_DEPT_FUNC_C3 CURSOR FOR 				
	SELECT A.L_DAY_ID, A.ORG_ID, A.L_WEEK_ID, A.AMT, A.CUST_NUM, A.CUST_AMT, 
