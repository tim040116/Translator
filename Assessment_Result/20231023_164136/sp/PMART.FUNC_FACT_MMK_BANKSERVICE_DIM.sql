REPLACE PROCEDURE PMART.FUNC_FACT_MMK_BANKSERVICE_DIM
(
...
SP:BEGIN
    ...
    IF CAST(LENGTH(TRIM(P_MMKID)) AS CHAR(2)) = '10' THEN
	    SET CONDWHERE = '   WHERE PARENT_MMK_ID+MMK_ID =  ''' + TRIM(P_MMKID)+''' ';	
	ELSE
	    SET CONDWHERE = '   WHERE PARENT_MMK_ID =  ''' + TRIM(P_MMKID)+''' ';
	END IF;
    CALL PMART.P_DROP_TABLE ('#VT_FUNC_FACT_MMK_BANKSERVICE_DIM');
    SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_FUNC_FACT_MMK_BANKSERVICE_DIM ( '
    ...
    SET SQLSTR = 'INSERT INTO #VT_FUNC_FACT_MMK_BANKSERVICE_DIM(PARENT_MMK_ID,MMK_ID,MMK_VAL,MMK_NAME,MMK_NO,ALLOW_DRILL_DOWN,MMK_LEVEL,DRILL_MMK_LEVEL,DRILL_MMK_ID) '
	                     +'SELECT * FROM( ';
    IF P_SHOWTYPE = 2 THEN 	
		   SET SQLSTR = SQLSTR +'  SELECT  CAST(DAT.PARENT_MMK_ID AS VARCHAR(10)) AS PARENT_MMK_ID  '
		                                    ...
		                                    +'     FROM PMART.VW_MMK_BANKSERVICE_DIM DAT '
    ...
    ELSE
		   SET SQLSTR = SQLSTR +'  SELECT CAST(''-2'' AS VARCHAR(10))  AS PARENT_MMK_ID '
		                                    ...
		                                    +'                 ,CAST(-1 AS SMALLINT)  AS MMK_LEVEL '
		                                    +'                 ,CAST(-2 AS SMALLINT)  AS DRILL_MMK_LEVEL '
