REPLACE PROCEDURE PMART.FUNC_FACT_REMD_0114
(
   IN P_RPTID SMALLINT,
...
CALL PMART.P_DROP_TABLE ('#VT_REMD_FACT');
    ...
    INSERT INTO PMART.T1(F1,F2) SELECT 20210114,SQLSTR;		
...
SET SQLSTR = ' CREATE MULTISET VOLATILE TABLE #VT_FACT_REMD_TMP AS('		  
		...
		INSERT INTO PMART.T1(F1,F2) SELECT 202101141,SQLSTR;
        EXECUTE IMMEDIATE SQLSTR;   
SET SQLSTR = ' CREATE MULTISET VOLATILE TABLE #VT_FACT_REMD AS('
		...
		INSERT INTO PMART.T1(F1,F2) SELECT 202101142,SQLSTR;		
        EXECUTE IMMEDIATE SQLSTR;   		
END SP;