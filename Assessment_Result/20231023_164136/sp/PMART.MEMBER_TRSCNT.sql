REPLACE PROCEDURE PMART.MEMBER_TRSCNT ()
...
SP:BEGIN
  ...
  SET SQLSTR =' SELECT DISTINCT  A.TIME_ID               ' +
              '   FROM PTEMP.TP202_FACT_MEMBER_TRSCNT A  ' +
              '  ' ;   
  ...
  DO    
     L1_1:
        BEGIN 
         ...
         INSERT INTO PTEMP.TP203_FACT_MEMBER_TRSCNT  
         (TIME_ID
         ,ORG_ID
         ,ACCU_TRSCNT
         ,ACCU_ADDCNT
         ...
         FROM (
                ...
                FROM (
                      SELECT TIME_ID,ORG_ID,MAX(TX_SEQ) AS TX_SEQ,MEMBER_ID,'1' AS TTYPE FROM (
						...
						FROM PTEMP.TP115_FACT_MEMBER_TRANS 
...
(
         ...
         FROM (
                ...
                FROM (
                      SELECT TIME_ID,ORG_ID,TX_SEQ,MEMBER_ID,'2' AS TTYPE FROM PTEMP.TP113_FACT_MEMBER_TRANS_ADD WHERE DWH_FG_ADD <> 0
                             ) AA  
              ) A INNER JOIN (SELECT OSTORE_ID,RESPON_ID,BRANCH_ID,DEPT_ID,PDEPT_ID 
                                FROM PMART.LATEST_ORG_DIM 
                               GROUP BY OSTORE_ID,RESPON_ID,BRANCH_ID,DEPT_ID,PDEPT_ID 
...
(
         ...
         FROM (
                ...
                FROM (
                      SELECT TIME_ID,ORG_ID,TX_SEQ,MEMBER_ID,'3' AS TTYPE FROM PTEMP.TP114_FACT_MEMBER_TRANS_EXG WHERE DWH_FG_EXG <> 0
                     ) AA  
              ) A INNER JOIN (SELECT OSTORE_ID,RESPON_ID,BRANCH_ID,DEPT_ID,PDEPT_ID 
                                FROM PMART.LATEST_ORG_DIM 
                               GROUP BY OSTORE_ID,RESPON_ID,BRANCH_ID,DEPT_ID,PDEPT_ID 
