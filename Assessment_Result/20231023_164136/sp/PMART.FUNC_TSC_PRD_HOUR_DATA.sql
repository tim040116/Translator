REPLACE  PROCEDURE PMART.FUNC_TSC_PRD_HOUR_DATA
(
)
SP:BEGIN
	...
	DELETE FROM PMART.TSC_SALES_PRD_DAY_24H;
	DELETE FROM PMART.TSC_SALES_PRD_DAY_8H;
	 SET SQLSTR ='INSERT INTO PMART.TSC_SALES_PRD_DAY_24H(DATA_DAY,DEPT_ID,DEPT_NAME,TSC_CODE,PRD_ID' +
                                ...
                                '   FROM PMART.TSC_SALES_PRD_DAY DATA' +
		                        '                 INNER JOIN PDATA.TSC_SETTING_PLAN_PRD T ON DATA.PRD_ID = T.PRD_ID '+
                                '                 INNER JOIN  PMART.LAST_ORG_STORE S ON S.OSTORE_ID = DATA.OSTORE_NO '+
                                '                 INNER JOIN PMART.ORG_DEPT D ON S.DEPT_ID = D.DEPT_ID	'+
                                ' WHERE  DATA.DATA_TYPE=2 '+
								'        AND (DATA.DAY_ID = '+P_DAY24 +
	       			            '                    OR DATA.DAY_ID = '+P_LW_DAY24+
								'                    OR DATA.DAY_ID = '+P_DAY+
								'                    OR DATA.DAY_ID = '+P_LW_DAY+
				                ') ' +
                                'GROUP BY   D.DEPT_NO,D.DEPT_NAME,T.TSC_CODE,DATA.PRD_ID';	 
	 EXECUTE IMMEDIATE SQLSTR;   
	 SET SQLSTR ='INSERT INTO PMART.TSC_SALES_PRD_DAY_8H(DATA_DAY,DEPT_ID,DEPT_NAME,TSC_CODE,PRD_ID' +
	                            '  ,TODAY_PRD_SALES00,LASTWEEK_PRD_SALES00,X_HEADER00 '+
								'  ,TODAY_PRD_SALES01,LASTWEEK_PRD_SALES01,X_HEADER01 '+
	                            '  ,TODAY_PRD_SALES02,LASTWEEK_PRD_SALES02,X_HEADER02 '+
	                            '  ,TODAY_PRD_SALES03,LASTWEEK_PRD_SALES03,X_HEADER03 '+
								...
								'                    T.TSC_CODE,'+
								'                    DATA.PRD_ID,'+								
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_DAY08 +'THEN SALES'+P_TIMERANGE08 +' ELSE 0 END) AS TODAY_PRD_SALES00,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_LW_DAY08 +'THEN SALES'+P_TIMERANGE08 +' ELSE 0 END) AS LASTWEEK_PRD_SALES00,' +
                                '                    CAST( '''+P_TIMERANGE08+''' AS VARCHAR(2)) AS X_HEADER00,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_DAY07 +'THEN SALES'+P_TIMERANGE07 +' ELSE 0 END) AS TODAY_PRD_SALES01,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_LW_DAY07 +'THEN SALES'+P_TIMERANGE07 +' ELSE 0 END) AS LASTWEEK_PRD_SALES01,' +
                                '                    CAST( '''+P_TIMERANGE07+''' AS VARCHAR(2)) AS X_HEADER01,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_DAY06 +'THEN SALES'+P_TIMERANGE06 +' ELSE 0 END) AS TODAY_PRD_SALES02,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_LW_DAY06 +'THEN SALES'+P_TIMERANGE06 +' ELSE 0 END) AS LASTWEEK_PRD_SALES02,' +
                                '                    CAST( '''+P_TIMERANGE06+''' AS VARCHAR(2)) AS X_HEADER02,' +			
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_DAY05 +'THEN SALES'+P_TIMERANGE05 +' ELSE 0 END) AS TODAY_PRD_SALES03,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_LW_DAY05 +'THEN SALES'+P_TIMERANGE05 +' ELSE 0 END) AS LASTWEEK_PRD_SALES03,' +
                                '                    CAST( '''+P_TIMERANGE05+''' AS VARCHAR(2)) AS X_HEADER03,' +		
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_DAY04 +'THEN SALES'+P_TIMERANGE04 +' ELSE 0 END) AS TODAY_PRD_SALES04,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_LW_DAY04 +'THEN SALES'+P_TIMERANGE04 +' ELSE 0 END) AS LASTWEEK_PRD_SALES04,' +
                                '                    CAST( '''+P_TIMERANGE04+''' AS VARCHAR(2)) AS X_HEADER04,' +			
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_DAY03 +'THEN SALES'+P_TIMERANGE03 +' ELSE 0 END) AS TODAY_PRD_SALES05,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_LW_DAY03 +'THEN SALES'+P_TIMERANGE03 +' ELSE 0 END) AS LASTWEEK_PRD_SALES05,' +
                                '                    CAST( '''+P_TIMERANGE03+''' AS VARCHAR(2)) AS X_HEADER05,' +								
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_DAY02 +'THEN SALES'+P_TIMERANGE02 +' ELSE 0 END) AS TODAY_PRD_SALES06,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_LW_DAY02 +'THEN SALES'+P_TIMERANGE02 +' ELSE 0 END) AS LASTWEEK_PRD_SALES06,' +
                                '                    CAST( '''+P_TIMERANGE02+''' AS VARCHAR(2)) AS X_HEADER06,' +					
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_DAY01 +'THEN SALES'+P_TIMERANGE01 +' ELSE 0 END) AS TODAY_PRD_SALES07,' +
                                '                    SUM(CASE WHEN  DATA.DAY_ID='+P_LW_DAY01 +'THEN SALES'+P_TIMERANGE01 +' ELSE 0 END) AS LASTWEEK_PRD_SALES07,' +
