REPLACE PROCEDURE PMART.ACCT2_FUNC
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_ACCT2_FUNC'); 
   CASE P_LEVEL
      WHEN 0 THEN  
           SET SQLSTR = 
              'CREATE MULTISET VOLATILE TABLE #VT_ACCT2_FUNC  AS('+
                   'SELECT '+
                   '-1 AS TOT_ID, '+
                   'ORG_ID AS ORG_ID, '+
                   ...
                   'ORG_ID IN (SELECT DISTINCT DEPT_ID FROM PMART.LATEST_ORG_DIM WHERE TOT_ID=-1) '+
