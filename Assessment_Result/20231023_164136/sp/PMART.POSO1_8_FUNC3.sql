REPLACE PROCEDURE PMART.POSO1_8_FUNC3
(
FP_DAY_ID  NUMBER,     
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_POSO1_8_FUNC3_1'); 
   CALL PMART.P_DROP_TABLE ('#VT_POSO1_8_FUNC3_2'); 
   CALL PMART.P_DROP_TABLE ('#VT_POSO1_8_FUNC3_3');
   CALL PMART.P_DROP_TABLE ('#VT_POSO1_8_FUNC3');   
  SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_POSO1_8_FUNC3_1  
                (    
                  DEPT_ID INTEGER
  ...
  EXECUTE IMMEDIATE SQLSTR;
    SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_POSO1_8_FUNC3_2  
                (    
                  V_ORG_ID INTEGER
		 ...
		 ,V_SALES_RATE NUMBER
                ...
                NO PRIMARY INDEX ON COMMIT PRESERVE ROWS;';   
  EXECUTE IMMEDIATE SQLSTR;
  SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_POSO1_8_FUNC3_3  
                (    
                  ORG_ID INTEGER
                ...
                NO PRIMARY INDEX ON COMMIT PRESERVE ROWS;';   
  EXECUTE IMMEDIATE SQLSTR;       
  SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_POSO1_8_FUNC3  
                (    
                  MESSAGE VARCHAR(200) Collate Chinese_Taiwan_Stroke_CI_AS
                )
                NO PRIMARY INDEX ON COMMIT PRESERVE ROWS;';   
  EXECUTE IMMEDIATE SQLSTR;
   INSERT INTO #VT_POSO1_8_FUNC3_1(DEPT_ID  ,DEPT_NM )
   ...
   SET SQLSTR =
      'INSERT INTO #VT_POSO1_8_FUNC3_2  '
      'SELECT O.DEPT_ID AS V_ORG_ID, O.DEPT_NM AS V_ORG_NM, ' +
      ...
      ';';
  EXECUTE IMMEDIATE SQLSTR;
  INSERT INTO #VT_POSO1_8_FUNC3_3(ORG_ID  ,DATA_1 )
  SELECT  V_ORG_ID
         ,CASE WHEN V_ORG_ID = -1 THEN
               '進' + TRIM(TO_CHAR(V_INPRD_CNT, '9999990')) + 
              ',銷' + TRIM(TO_CHAR(V_SALES_CNT, '9999990')) + ',比' + TRIM(TO_CHAR(V_SALES_RATE, '9999990.00')) + '%'
          ELSE
               V_ORG_NM + 
              ':銷' + TRIM(TO_CHAR(V_SALES_CNT, '9999990')) + ',比' + TRIM(TO_CHAR(V_SALES_RATE, '9999990')) + '%'
          END AS DATA_1
  FROM #VT_POSO1_8_FUNC3_2;  
  SET SQLSTR = ' SELECT DATA_1 FROM #VT_POSO1_8_FUNC3_3 ORDER BY ORG_ID ' ;						  
  ...
  DO    
     ...
     END L1_1;     
  END WHILE L1;
  INSERT INTO #VT_POSO1_8_FUNC3 (MESSAGE) VALUES (V_DATA_1);	 
  CLOSE STORE_CS;     
END SP;