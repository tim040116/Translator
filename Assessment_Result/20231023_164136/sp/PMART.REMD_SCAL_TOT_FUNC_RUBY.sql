REPLACE  PROCEDURE PMART.REMD_SCAL_TOT_FUNC_RUBY (P_DAY_ID NUMBER)
BEGIN
DECLARE SQLSTR  VARCHAR(5000) DEFAULT ''; 
  ...
  SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_REMD_SCAL_TOT_FUNC_1 ,NO FALLBACK,NO JOURNAL,NO LOG 
   (
        R14    VARCHAR(20) CHARACTER SET UNICODE CASESPECIFIC ,
        R15    VARCHAR(8) CHARACTER SET UNICODE CASESPECIFIC 
    )   
    ON COMMIT PRESERVE ROWS;';  
  EXECUTE IMMEDIATE SQLSTR; 
  INSERT INTO #VT_REMD_SCAL_TOT_FUNC_1(R14  ,R15 ) SELECT '僅課別以下顯示' ,NULL ; 
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_SCAL_TOT_FUNC, NO FALLBACK,NO JOURNAL,NO LOG AS('||            
                'SELECT '||
                'TOT_ID, '||
                'ORG_ID, '||
