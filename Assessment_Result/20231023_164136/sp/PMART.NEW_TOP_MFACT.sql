REPLACE PROCEDURE PMART.NEW_TOP_MFACT
...
FP_PRD_TYPE CHAR(1),FP_PRD_LEVEL NUMBER,FP_PRD_ID NUMBER)
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_NEW_TOP_MFACT'); 
   CASE
      WHEN FP_TIME_TYPE='D' AND FP_ORG_LEVEL=2 AND FP_PRD_TYPE='P' AND FP_PRD_LEVEL=3  THEN  
            SET V_TABLE_NAME = 'PMART.BASIC_MFACT_DETAIL';             
      WHEN FP_TIME_TYPE='D' AND FP_ORG_LEVEL=2 AND FP_PRD_TYPE='R' AND FP_PRD_LEVEL=2  THEN  
            SET V_TABLE_NAME = 'PMART.BASIC_MFACT_DETAIL';
   ...
   END CASE;
   SET SQLSTR1 = 'CREATE MULTISET VOLATILE TABLE #VT_NEW_TOP_MFACT  AS('+
                'SELECT '+
