REPLACE PROCEDURE PMART.REMD_TT_NCIG_OSTORE_FUNC(P_DAY_ID NUMBER,P_RESPON_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(8000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_TT_NCIG_OSTORE_FUNC'); 
  CALL PMART.REMD_T1_NCIG_OSTORE_FUNC(P_DAY_ID,P_RESPON_ID);
  CALL PMART.REMD_T2_NCIG_OSTORE_FUNC(P_DAY_ID,P_RESPON_ID);
  CALL PMART.REMD_T3_NCIG_OSTORE_FUNC(P_DAY_ID,P_RESPON_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_TT_NCIG_OSTORE_FUNC AS('+            
               'SELECT '+
               '-1 AS TOT_ID, '+
               'A.ORG_ID AS ORG_ID, '+
               ...
               'FROM '+
               '#VT_REMD_T1_NCIG_OSTORE_FUNC A '+
               ' LEFT JOIN '+
               '#VT_REMD_T2_NCIG_OSTORE_FUNC B '+
               '   ON (A.ORG_ID=B.ORG_ID) '+
               ' LEFT JOIN '+
               '#VT_REMD_T3_NCIG_OSTORE_FUNC C '+
