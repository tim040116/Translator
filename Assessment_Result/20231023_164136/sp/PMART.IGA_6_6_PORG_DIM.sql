REPLACE PROCEDURE PMART.IGA_6_6_PORG_DIM
(
...
SP:BEGIN
    ...
    CALL PMART.P_DROP_TABLE ('#VT_IGA_6_6_PORG_DIM');
    SET SQLSTR = 'CREATE MULTISET VOLATILE TABLE #VT_IGA_6_6_PORG_DIM ( '
	...
	SET SQLSTR_UNION = ' SELECT DISTINCT O.RESPON_ID AS PARENT_ORG_ID,O.STORE_ID AS ORG_ID, O.STORE_ID AS ORG_VAL, (O.STORE_NO+''_''+O.STORE_NM) AS ORG_SNAME,O.STORE_NO (VARCHAR(11)) AS ORG_NO,''N'' AS ALLOW_DRILL_DOWN,4 AS ORG_LEVEL '
							   ...
							   +' INNER JOIN PMART.FACT_PBOA_EXCL_PRD C '
							   +' ON O.STORE_ID = C.ORG_ID '
							   +' UNION '
    ...
    SET SQLSTR = 'INSERT INTO #VT_IGA_6_6_PORG_DIM(PARENT_ORG_ID,ORG_ID,ORG_VAL,ORG_SNAME,ORG_NO,ALLOW_DRILL_DOWN,ORG_LEVEL,DRILL_ORG_LEVEL,DRILL_ORG_ID) '
	                     +'SELECT * FROM( ';
