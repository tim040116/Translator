REPLACE PROCEDURE PMART.REMD_S4_DEPT_FUNC(P_DAY_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_S4_DEPT_FUNC'); 
  ...
  LOCKING PMART.LAST_ORG_DIM FOR ACCESS;  
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_S4_DEPT_FUNC AS('+            
          'SELECT '+
          'B.ORG_ID AS ORG_ID, '+
          ...
          ' INNER JOIN '+            
		  '(SELECT ORG_ID ,MASK FROM PMART.LAST_ORG_DIM_MASK WHERE P_ORG_ID >= 3100000     AND P_ORG_ID <= 3199999) B '+
          ' ON 1 = 1 '+
          ' LEFT OUTER JOIN '+
