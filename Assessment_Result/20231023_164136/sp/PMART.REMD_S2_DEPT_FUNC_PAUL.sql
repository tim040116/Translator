REPLACE PROCEDURE PMART.REMD_S2_DEPT_FUNC_PAUL(P_DAY_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_S2_DEPT_FUNC_PAUL'); 
  LOCKING PMART.REMD_FACT_SUM FOR ACCESS;
  LOCKING PMART.LAST_ORG_DIM FOR ACCESS;
  LOCKING PMART.YMWD_TIME FOR ACCESS;
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_S2_DEPT_FUNC_PAUL AS('+            
               'SELECT '+
               'B.DEPT_ID AS ORG_ID, '+
