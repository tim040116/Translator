REPLACE PROCEDURE PMART.REMD_TCAL_NCIG_PDEPT_FUNC(P_DAY_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_TCAL_NCIG_PDEPT_FUNC'); 
  CALL PMART.REMD_TT_NCIG_PDEPT_FUNC(P_DAY_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_TCAL_NCIG_PDEPT_FUNC AS('+            
                'SELECT '+
                'TOT_ID, '+
                'ORG_ID, '+
                ...
                'FROM #VT_REMD_TT_NCIG_PDEPT_FUNC T1 '+
