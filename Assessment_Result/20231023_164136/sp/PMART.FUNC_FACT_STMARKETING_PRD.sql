REPLACE PROCEDURE PMART.FUNC_FACT_STMARKETING_PRD
(
   IN P_ACTID INTEGER,
...
SP:BEGIN
	...
	ELSE
		SET SELCOL = 'T1.TIME_ID,T1.ACT_ID, CAST(PRD.FM_CODE AS VARCHAR(7)) AS ID, PRD.FM_NAME AS NM';
	...
	ELSE
		 SET SQLSTR = SQLSTR +'     INNER JOIN PDATA.STORMKT_PRD_D PRD ON T1.ACT_ID = PRD.ACT_ID AND T1.CLASS_ID = PRD.CLASS_ID AND T1.PRD_ID = CAST(PRD.FM_CODE AS VARCHAR(7)) ';
	END IF;
