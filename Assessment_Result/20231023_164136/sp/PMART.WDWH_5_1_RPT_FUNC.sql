REPLACE PROCEDURE PMART.WDWH_5_1_RPT_FUNC
(
   IN I_ORG_TYPE  VARCHAR(2),   
   IN I_ORG_ID    INTEGER,      
   IN I_DAY_ID    INTEGER,      
   IN I_KND_ID    VARCHAR(2),     
   IN I_GRP_ID    VARCHAR(3)       
)
 ...
 END IF;
  ...
  CALL PMART.P_DROP_TABLE ('#VT_WDWH_5_1_RPT_FUNC_RPT');
   ...
   IF (I_ORG_TYPE = '3') THEN	
			CREATE MULTISET VOLATILE TABLE #VT_WDWH_5_1_RPT_FUNC_ORG AS(	
                    SELECT DISTINCT -1 AS ORG_LEVEL, TO_CHAR(TOT_ID)    AS ORG_ID FROM PMART.LAST_ORG_DIM 
                    UNION ALL
                    SELECT DISTINCT  3 AS ORG_LEVEL, TO_CHAR(RESPON_ID) AS ORG_ID FROM PMART.ORG_DIM_POSI1  WHERE RESPON_ID = '' + I_ORG_ID + ''
                    UNION ALL
    ...
    IF (I_ORG_TYPE = '5') THEN
	        IF I_GRP_ID = 0 THEN
					CREATE MULTISET VOLATILE TABLE #VT_WDWH_5_1_RPT_FUNC_ORG AS(	
		                    ...
		                    SELECT DISTINCT  3 AS ORG_LEVEL, TO_CHAR(RESPON_ID) AS ORG_ID FROM PMART.ORG_DIM_POSI1  WHERE RESPON_ID = '' + I_ORG_ID + ''
		                    UNION ALL
		                    SELECT DISTINCT  6 AS ORG_LEVEL, CAST(B.FM_CODE AS VARCHAR(7))  AS ORG_ID 
