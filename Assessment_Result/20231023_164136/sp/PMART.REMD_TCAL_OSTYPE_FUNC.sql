REPLACE PROCEDURE PMART.REMD_TCAL_OSTYPE_FUNC(P_DAY_ID NUMBER,P_ORG_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(4000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_TCAL_OSTYPE_FUNC'); 
  CALL PMART.REMD_TT_OSTYPE_FUNC(P_DAY_ID,P_ORG_ID);
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_TCAL_OSTYPE_FUNC AS('+            
                 'SELECT '+
                 'TOT_ID, '+
                 'STYPE_ID, '+
                 ...
                 'FROM #VT_REMD_TT_OSTYPE_FUNC T1 '+
