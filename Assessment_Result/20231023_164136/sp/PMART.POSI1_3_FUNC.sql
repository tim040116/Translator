REPLACE PROCEDURE PMART.POSI1_3_FUNC (P_L_DAY_ID VARCHAR(400), P_LEVEL VARCHAR(2), P_PRD_ID VARCHAR(400) )
SQL SECURITY INVOKER
SP:BEGIN
   DECLARE SQLSTR  VARCHAR(8000); 	
   DECLARE V_TABLE_NAME VARCHAR(30);
   CALL PMART.P_DROP_TABLE ('#VT_POSI1_3_FUNC'); 
   CALL PMART.P_DROP_TABLE ('#VT_POSI_FUNC');
   CALL PMART.P_DROP_TABLE ('#VT_POSI1_3_FUNC_P'); 
   CALL PMART.POSI_FUNC(P_L_DAY_ID ,P_LEVEL,P_PRD_ID);   
   CALL PMART.POSI_SHORT_FUNC('MONTH_PRE',P_L_DAY_ID,P_LEVEL,P_PRD_ID);
   SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_3_FUNC_P  AS('+
                   'SELECT '+
                   'M.TIME_ID, '+
                   'M.PRD_ID, '+
                   'M.SALES_AMT, '+
   ...
   SET SQLSTR ='CREATE MULTISET VOLATILE TABLE #VT_POSI1_3_FUNC  AS('+
                   'SELECT '+
                   'S.TIME_ID, '+
                   'S.PRD_ID, '+
                   'S.SALES_AMT, '+
                   ...
                   'S.PSD_DIFF, '+
                   'CASE WHEN L.REAL_SALES_AMT_PSD = 0   THEN 0 ELSE S.REAL_SALES_AMT_PSD/L.REAL_SALES_AMT_PSD*100   END AS L_YEAR_PSD_DIFF, '+
                   'CASE WHEN S.P_REAL_SALES_AMT_PSD = 0  THEN 0 ELSE S.REAL_SALES_AMT_PSD/S.P_REAL_SALES_AMT_PSD*100 END AS L_MONTH_PSD_DIFF, '+
                   'S.SALES_COST, '+
                   'S.GP_SALES_AMT, '+
                   'S.GP_SALES_AMT2, '+
                   'S.GP_SALES_AMT_PSD, '+
