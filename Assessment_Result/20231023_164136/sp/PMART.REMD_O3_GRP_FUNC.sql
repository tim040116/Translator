REPLACE PROCEDURE PMART.REMD_O3_GRP_FUNC
(P_L_DAY_ID NUMBER,P_GRP_ID NUMBER)
SQL SECURITY INVOKER
SP:BEGIN
DECLARE SQLSTR  VARCHAR(8000) DEFAULT ''; 
  CALL PMART.P_DROP_TABLE ('#VT_REMD_O3_GRP_FUNC'); 
      SET SQLSTR = 
          'CREATE MULTISET VOLATILE TABLE #VT_REMD_O3_GRP_FUNC AS('+            
                'SELECT '+
                ...
                'FROM '+
                '(SELECT * '+
                'FROM PMART.REMD_FACT WHERE L_DAY_ID=ROUND('+P_L_DAY_ID+'/100)*100+1-10000) A, '+
