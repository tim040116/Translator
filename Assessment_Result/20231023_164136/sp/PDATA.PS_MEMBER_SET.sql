REPLACE  PROCEDURE PDATA.PS_MEMBER_SET()
SP:BEGIN
	DECLARE V_TRG_PERSON SMALLINT;
	DECLARE V_SQL VARCHAR(20000);
	DECLARE V_JOB_ID INTEGER;
	SET V_JOB_ID = (SELECT JOB_ID FROM PDATA.RANDOM_ETL WHERE KEY_ID='PRDREL');
	SET V_TRG_PERSON = (SELECT TRG_PERSON FROM PDATA.PRDREL_SET WHERE JOB_ID = V_JOB_ID);
	IF V_TRG_PERSON = 2 THEN
		...
		EXECUTE IMMEDIATE V_SQL;
	 	SET V_SQL = 
		'UPDATE PTEMP.PS_MEM SET AGES = 100  WHERE BIRTHDAY IS NOT NULL AND AGES=0';
		EXECUTE IMMEDIATE V_SQL;
	 	SET V_SQL = 
		...
		EXECUTE IMMEDIATE V_SQL;
	 	SET V_SQL = 
		'DELETE PTEMP.PS_TRANS_PRODUCE_DETAIL
		WHERE TX_SEQ NOT IN (
			SELECT TX_SEQ 
			FROM #VT_TRANS_MEM A 
			  INNER JOIN PDATA.PRDREL_CAGE_SET B ON A.AGES BETWEEN B.AGE_BGN AND AGE_END
