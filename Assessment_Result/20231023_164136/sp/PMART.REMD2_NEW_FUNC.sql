CREATE PROCEDURE PMART.REMD2_NEW_FUNC
...
SP:BEGIN
   ...
   CALL PMART.P_DROP_TABLE ('#VT_REMD2_NEW_FUNC'); 
   ...
   CASE P_LEVEL
      WHEN -1 THEN  
           SET SQLSTR = 
              'CREATE MULTISET VOLATILE TABLE #VT_REMD2_NEW_FUNC  AS('+
                      'SELECT  '+
                      '-1 AS TOT_ID, '+
                      'B.ORG_ID AS ORG_ID, '+
        ...
        WHEN 0 THEN  
           SET SQLSTR = 
              'CREATE MULTISET VOLATILE TABLE #VT_REMD2_NEW_FUNC  AS('+
                      ...
                      '   SELECT  * '+
                      '   FROM PMART.REMD_FACT_MONTH_SUM '+
                      '   WHERE ORG_ID IN (SELECT DISTINCT DEPT_ID FROM PMART.LATEST_ORG_DIM WHERE PDETP_ID='+P_ORG_ID+') '+
                      '   AND L_MONTH_ID='+P_L_MONTH_ID +
                      '   ) B '+
                      'LEFT JOIN '+
                      '   ( '+
	  ...
	  WHEN 1 THEN  
           SET SQLSTR = 
              'CREATE MULTISET VOLATILE TABLE #VT_REMD2_FUNC  AS('+
                    'SELECT  '+
                    '-1 AS TOT_ID, '+
                    'B.ORG_ID AS ORG_ID, '+
                    'CAST(B.TOT_AMT AS DECIMAL(16,6))/DECODE(B.TOT_PLAN_STNUM,0,NULL,B.TOT_PLAN_STNUM) AS AVG_AMT, '+
