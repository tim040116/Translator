REPLACE   PROCEDURE PMART.PS_TRANS
(
)
SP:BEGIN
	...
	SET P_TX_EDT = '''' + (SELECT TX_EDT + ' 23:59:59' AS TX_EDX FROM PDATA.PRDREL_SET WHERE  JOB_ID = P_JOB_ID) + '''';
 	CALL PMART.P_DROP_TABLE ('#VT_PS_TRANS');
SET SQLSTR = ' 
SELECT * FROM PDATA.TRANS
	WHERE  TX_DTTM BETWEEN  '+ P_TX_SDT + ' AND '+P_TX_EDT+'   
	AND EXTRACT(HOUR FROM TX_DTTM) IN 
		(
		SELECT TIME_SEQ 	
	...
	AND OSTORE_ID IN 
		(
	SELECT * FROM SYSDBA.#VT_PS_OSTORE
		)
'
; 
   SET SQLCREATE = 
   'CREATE MULTISET VOLATILE TABLE #VT_PS_TRANS AS( '
   +SQLSTR 
