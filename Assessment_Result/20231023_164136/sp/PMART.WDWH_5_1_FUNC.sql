REPLACE PROCEDURE PMART.WDWH_5_1_FUNC(P_LEVEL INTEGER,P_U_DEPT INTEGER,P_DATE INTEGER,P_KND_ID VARCHAR(2) , P_GRP_ID VARCHAR(3))
...
SP:BEGIN
	...
	IF P_LEVEL = 3 THEN  
			...
			SET MSQLSTR_51 = '	CREATE MULTISET VOLATILE TABLE #VT_WDWH_5_1_FUNC_1 AS ( '
										  ...
										  + '		LEFT  JOIN PMART.BASIC_MFACT_DETAIL B4 ON B4.PRD_ID = CAST(B1.PRD_NO AS VARCHAR(7))  AND B4.TIME_ID = ' + PROCEDATE
			...
			SET MSQLSTR_53  = ' CREATE MULTISET VOLATILE TABLE #VT_WDWH_5_1_FUNC_3 AS	( '
										   ...
										   + '			INNER JOIN  PMART.PCB_V_PRDANTICIPATE AS B ON B.PUBLISH_DATE_DATE =  ' '''' +PROCSDATE +''''  'AND  A.PRD_ID = CAST(B.PRD_NO AS VARCHAR(7))'
	...
	IF P_LEVEL = 4 THEN  
			...
			SET MSQLSTR_53  = ' CREATE MULTISET VOLATILE TABLE #VT_WDWH_5_1_FUNC_3 AS	( '
			                               + ' 		SELECT  PRD_ID, ISNULL(SUM(SALES_CNT)) AS W2_SALES_CNT '
										   + '			FROM PMART.BASIC_MFACT_DETAIL AS A  '
										   + '			INNER JOIN  (SELECT DISTINCT PRD_NO FROM #VT_WDWH_5_1_FUNC_1 )  AS B ON A.PRD_ID = CAST(B.PRD_NO AS VARCHAR(7)) '
										   + '			WHERE A.TIME_ID >= ' + P_DATE + ' AND A.TIME_ID <= '+ PROCEDATE 
										   + '		    GROUP BY PRD_ID   '
										   + '  ) WITH DATA NO PRIMARY INDEX ON COMMIT PRESERVE ROWS; 	';		
			EXECUTE IMMEDIATE MSQLSTR_53; 	
