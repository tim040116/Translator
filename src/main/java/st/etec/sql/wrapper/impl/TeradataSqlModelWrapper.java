package st.etec.sql.wrapper.impl;
 ...
 * 
 */
public class TeradataSqlModelWrapper extends SqlModelWrapper {
	...
	public CreateTableModel createTable(String sql) {
		...
		for (String c : tempsql.split("")) {
			...
			case 3:// column
				...
				if (",".equals(c) && parenthesesCnt == 1 || parenthesesCnt == 0) {
					...
					col.setSetting(temp.replaceAll(arCol[0], "").replaceAll(arCol[1], "").trim());
