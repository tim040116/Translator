...
//				.replaceAll("[Ee][Xx][Tt][Rr][Aa][Cc][Tt] *\\( *[Yy][Ee][Aa][Rr] *[Ff][Rr][Oo][Mm]", "DatePart(year ,")//all
		...
		String tssql = DQLTransducer.transduceSelectSQL(content);
		content = strTry;
		int cntarea = 1;
...
//			}
			...
			if (sql.matches("DROP\\s+TABLE\\s+\\S+\\s*;")) {
				res += DDLTransducer.runDropTable(sql)+"\r\n";
			}
			//drop view
...
//			}
				...
				sql = DQLTransducer.changeSample(sql);
				sql = DQLTransducer.transduceSelectSQL(sql);
				res += sql + "\r\n\r\n";
			}
		}
