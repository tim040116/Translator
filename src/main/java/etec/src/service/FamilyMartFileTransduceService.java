	...
	public static String transduceSQLScript(String content)throws UnknowSQLTypeException , IOException {
		...
		switch(sqlType) {
			case TRUNCATE_TABLE:
				break;
			case REPLACE_VIEW:
				result = DDLTransducer.runReplaceView(result);
				break;
			case DROP_VIEW:
			case DROP_TABLE:
				result = DDLTransducer.runDropTable(result);
				break;
			case RENAME_TABLE:
				result = DDLTransducer.runRenameTable(result);
				break;
			case COLLECT_STATISTICS:
				result = DDLTransducer.runStatistics(result);
				break;
			case CREATE_TABLE:
				result = DDLTransducer.runCreateTable(result);
				break;
			case CREATE_INSERT:
			...
			case INSERT_TABLE:
				...
				break;
			case INSERT_SELECT:
				result = DMLTransducer.runInsertSelect(result);
				break;
			case MERGE_INTO:
				result = DMLTransducer.runMergeInto(result);
				break;
			case SELECT_TABLE:
				result = DQLTransducer.transduceSelectSQL(result);
				break;
			case UPDATE_TABLE:
				result = DQLTransducer.transduceSelectSQL(result);
				break;
